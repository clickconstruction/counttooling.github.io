...
- **Persisted settings** (localStorage): `counterSettings`, `lineTypeSettings`, `exportSettings`, `recentLineColors`, `iconNames`, `iconOrder`, `pageScales`, `specificPagesIncludeReport`, `bundleHighlightsToPdf` (in exportSettings); include new fields in export/import when adding
...
- **Modals**: Project Settings (gear) — Save/Load/Close Project, Manage Projects (admin), Manage Icons, Export, Import; User Settings (user icon) — email, password, Add User / Manage User (admin), All Users (admin), Sign Out; Manage User modal — list users with Delete per row; Manage Projects modal — list all projects with Delete per row; Specific Pages modal — thumbnails, per-page marked/unmarked/exclude, bulk actions, Include takeoff report; pipeToolingCopiedModal — "Copied to clipboard" toast (1.5s auto-dismiss); Choose Line Type modal — tabs Choose Line Type / Create Line Type; macrosModal — Keyboard Shortcuts (M/S/C/L/P/D/Esc/arrows/Enter), opened via status bar "Macros" link
...
- **Hotkeys** — M (Move), S (Set Scale), C (Counter), L (Quick Line), P (Polyline), D (Measure Distance); ignored when focus is in input/textarea/contenteditable
