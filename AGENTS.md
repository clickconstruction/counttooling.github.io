...
- **Persisted settings** (localStorage): `counterSettings`, `lineTypeSettings`, `exportSettings`, `recentLineColors`, `iconNames`, `iconOrder`, `pageScales`, `zoomSettings`, `specificPagesIncludeReport`, `bundleHighlightsToPdf`, `bundleNotesToPdf` (in exportSettings), `clickcount-last-project` (last-project restore metadata); `maxZoom` per project in save/load; include new fields in export/import when adding
