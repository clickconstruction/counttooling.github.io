...
- **Copy to /Tooling** — Button below Specific Pages; copies tab-delimited summary (fixture, count, page) to clipboard via `getPipeToolingSummary()`; counters as name/count/pages; line types as `[unit] of [name]`/length/pages; hidden when no counts or lines
