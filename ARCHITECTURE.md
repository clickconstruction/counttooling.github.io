...
Current first-party line counts (`wc -l`, 2026-09-06 — the **numbers and this
...
- **View links** — `project_view_links` + `view_link_access_log`; Share modal
  ...
  (returns `updatedAt`); email domain gate — or a **viewer grant** (`&g=`, 2026-09-06:
  `_shared/viewGrant.mjs` verified against `PT_VIEW_GRANT_SECRET`; PipeTooling's sub portal
  mints it so a sub opens plans with no email prompt; the log records `viewer_name`/`source`);
  `initViewOnlyMode`. The shared
