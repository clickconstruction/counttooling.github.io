...
| `features/*.js` (55 files) | 12,404 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
...
- **Sheet-size correction (compressed-PDF fix)** — the architectural presets and
  ...
  **Page scale only** — never scale zones (they inherit page scale) and never the
  two-point "Select on PDF" path (already ground truth, no `72`). Because the corrected
