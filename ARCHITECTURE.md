...
| Quick Plumbing | `plumModal` or `plumBtn` or `populatePlumModal` |
...
| Grid overlay | `showGridOverlay` or `gridSettingsModal` or `drawGrid` or `snapToGrid`; modifiers: opacity, color, lineWidth, lineStyle, offsetX/Y, majorInterval, snapToGrid |
...
- **Grid overlay** — Overlay on canvas with spacing grid; `state.showGridOverlay` (default false); grid button toggles; when enabled, prompts for spacing in `gridSettingsModal`; `state.gridSettings` has `{ spacing, unit, offsetX, offsetY, opacity, color, lineWidth, lineStyle, majorInterval?, snapToGrid }`; modifiers: opacity (0–1), color (hex), line width (px), line style (solid/dashed/dotted), offset X/Y (real-world units), major line every N lines, snap counters to grid; `drawGrid` draws in PDF-space using page scale; `snapToGrid(pdf, pageIdx)` snaps counter placement when enabled; grid is view-only (not exported to PDF); persisted in save/load
