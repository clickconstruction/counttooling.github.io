...
Current first-party line counts (`wc -l`, 2026-08-30 — the **numbers and this
...
|------|------:|------------------|
| [app.js](app.js) | 7,432 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
...
| [canvas-draw.js](canvas-draw.js) | 961 | Done — the unified annotation draw core (`createCanvasDraw(deps)` + `drawAnnotationsCore`), node-tested, guarded by [render-pixels.spec.js](render-pixels.spec.js). Both draw paths are thin env-builders over it. |
| [app/index.html](app/index.html) | 2,792 | The shell: HTML structure + every modal, no inline JS. Flat markup with no build step to split it; grows roughly linearly with modal count. Leave. |
| [styles.css](styles.css) | 1,738 | All CSS, token-organized. Leave. |
| [features/load-project.js](features/load-project.js) | 715 | Largest feature file (Load Project modal + filters), split 2026-07-30: the copy/fork domain moved to [features/copy-project.js](features/copy-project.js) at the file's documented domain boundary, and the row renderer was decomposed along its action boundaries (size / row HTML / actions / admin access / load click). Healthy — leave. |
...
| `features/*.js` (68 files) | 16,097 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
...
| [canvas-draw.test.js](canvas-draw.test.js) | Node `node:test` unit tests for [canvas-draw.js](canvas-draw.js) — a recording 2D-context Proxy stub + stubbed `Path2D`, geometry/icons globals via `Object.assign(globalThis, require(...))`: `hexToRgb` parse/fallback, `lineStyleToDash`, `drawDropMarker` glyph/save-restore/default color, room boxes (label vs "no scale" vs tiny-box skip), `drawAnnotationsCore` env invariants — selection glow doubles width + shadow (live) vs absent (export), `fontFamily`/`fontScale` flow into labels and notes while counter numbers stay `DM Sans`, note handles gated on `drawNoteHandles`, group dots at `env.dotRadius`, hollow vs solid counter rings + outline, and the frozen paint order — plus the `drawLegend`/`drawGrid` region: legend rows (multiply-zone counts, ft/px splits, room volumes), the Tier-3 B10 legend behaviors (empty legend paints **nothing** + `legendHasRows` mirrors the gate; the "This sheet" scope header paints first, above the rows; off-page/right-edge anchors are walked back on-sheet at ideal width), and grid gating/line counts; run with `npm run test:unit` |
...
| [legend-proof.spec.js](legend-proof.spec.js) | Playwright regression for the **Tier-3 B10 legend & proof-surface batch** (J8 J18) — an empty legend paints nothing (blank `#annCanvas` on a zero-mark sheet) and is not hittable (a drag in its rect pans the sheet; seeding a marker brings paint + drag back); the legend **wins the hit contest over an overlapping highlight** (real drag moves the legend, pan and highlight untouched); after `R` the page-coords anchor is walked back fully onto the rotated sheet at unreduced width (landscape-park → 180° repro); the Summary heading tooltip says "Legend settings — ▼ collapses" and the click still opens Legend Settings; the footer totals carry inline words ("counts" / "of lines", CSS-hidden below 1280px so the one-line bar survives) with click → `#summarySection` scroll + `.summary-flash`; and `R` is ignored while `#summaryCountDetailModal` is open, working again once closed. Asserts no console / page errors; `npx playwright test legend-proof.spec.js` |
...
- L186 - State
- L370 - [sync] Sync recovery & client recycle
- L451 - [sync] Global force reload
- L535 - [sync] Save Status log & envelope
- L538 - [sync] Field-error telemetry
- L597 - [sync] Dirty tracking & local session reset
- L603 - Undo/redo stacks
- L743 - [sync] Checkout probe, hashing & PDF cache
- L805 - Math & Format Helpers
- L1227 - Coordinate Helpers
- L1235 - PDF render bitmap cache
- L1289 - Sharp crop tile (deep-zoom sharpening + window-first commits)
- L1300 - PDF Rendering
- L2060 - UI Render Functions
- L2682 - Inline rename & polyline edit mode
- L2796 - Modal primitives (showModal / hideModal)
- L2827 - Toasts & line color picker
- L2895 - Airboard cloud sync
- L2940 - Supabase RPC & presence heartbeat
- L2980 - User activity / event telemetry
- L3039 - Supabase auth & dev auth
- L3225 - [sync] Checkout subscription & permission refresh
- L3235 - Modals & Handlers
- L3303 - PDF intake (upload, test PDF, hashing)
- L3311 - Toolbar tool buttons
- L3511 - Tool sidebar buttons & legend overlay
- L3602 - Add Line Type modal
- L3685 - Line color & sidebar handlers
- L3894 - Polyline modal & drawing
- L3937 - Zoom bar & page navigation
- L3963 - Export canvas JSON
- L3979 - PDF download helpers
- L3988 - View-link URL helpers & show-highlights/notes
- L4060 - Custom icon upload handler
- L4070 - Export & report dropdown menus
- L4157 - Sidebar drawer toggles
- L4188 - Mobile actions burger menu pointer & header logo
- L4200 - User Activity pointer (format.js + features/user-activity.js)
- L4212 - My Settings pointer (features/my-settings.js)
- L4237 - Auth & settings entry buttons
  - L4296 - Project Settings checkout & Save Status bell
  - L4388 - [sync] Checkout expired recovery
  - L4444 - [sync] Turn In
  - L4553 - Share modal pointer & copy-project openers
  - L4584 - Settings menu actions
  - L4605 - Auth sign-in form
  - L4630 - Save Project modal
  - L4643 - Checkout expired recovery modal wiring
  - L4748 - Last-session restore prompt
  - L4755 - Canvas Repair modal wiring
- L4942 - Canvas Event Handlers
- L5394 - Event Binding
- L5404 - Aim loupe (mobile press-hold precise placement)
- L5554 - Zoom transform preview & commit
- L5633 - Canvas mouse, wheel & touch handlers
- L6385 - Global dropdown dismissal & keyboard hotkeys
- L6732 - [sync] Manual save to cloud
- L6742 - [sync] Auto-save
- L6749 - [sync] Local backup (IndexedDB takeoff state)
- L6882 - [sync] Checkout keep-alive
- L6896 - App feature registry
- L7211 - View-only mode
- L7217 - Init / boot
