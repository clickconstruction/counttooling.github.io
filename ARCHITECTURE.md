...
|------|------:|------------------|
| [app.js](app.js) | 7,375 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
| [save-engine.js](save-engine.js) | 2,948 | Done — the extracted save/sync seam module (Stages 1–6), 44 node tests. Large but modular and fully node-testable; no further action. |
...
| [styles.css](styles.css) | 1,722 | All CSS, token-organized. Leave. |
...
| `features/*.js` (68 files) | 16,091 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
...
| [features/copy-project.js](features/copy-project.js) | The **copy/fork domain + save-before-load gate**, split out of [features/load-project.js](features/load-project.js) 2026-07-30 at that file's documented domain boundary (the code itself arrived from app.js as registry split #35). Owns `pendingCopyProject` / `copyProjectModalTarget`; binds `#copyProjectModalConfirm` + the `#saveBeforeLoad*` modal buttons at load; registers `App.openCopyProjectModalOrPromptSave`, `App.openLoadProjectModalOrPromptSave`, `App.hydrateProjectFromCloudRow`, `App.resolvePdfBufferForCloudProject`, `App.buildPagesFromPdfArrayBufferAndProjectData`, `App.resetCopyProjectState`, `App.clearCopyProjectModalTarget`. Reads `App.openLoadProjectModal` (registered by load-project.js) at call time, so load order between the two files is irrelevant. `buildPagesFromPdfArrayBufferAndProjectData` seeds plan-name default page labels (`planName` param from the load/fork callers — never the old hardcoded "document.pdf"); the payload's saved labels, incl. owner renames, override them via `applyPageAnnotationsFromData`. Regression: [copy-project.spec.js](copy-project.spec.js) + [page-label-roundtrip.spec.js](page-label-roundtrip.spec.js). |
...
- L176 - State
- L360 - [sync] Sync recovery & client recycle
- L441 - [sync] Global force reload
- L525 - [sync] Save Status log & envelope
- L528 - [sync] Field-error telemetry
- L587 - [sync] Dirty tracking & local session reset
- L593 - Undo/redo stacks
- L733 - [sync] Checkout probe, hashing & PDF cache
- L795 - Math & Format Helpers
- L1211 - Coordinate Helpers
- L1219 - PDF render bitmap cache
- L1273 - Sharp crop tile (deep-zoom sharpening + window-first commits)
- L1284 - PDF Rendering
- L2044 - UI Render Functions
- L2663 - Inline rename & polyline edit mode
- L2777 - Modal primitives (showModal / hideModal)
- L2808 - Toasts & line color picker
- L2876 - Airboard cloud sync
- L2921 - Supabase RPC & presence heartbeat
- L2961 - User activity / event telemetry
- L3020 - Supabase auth & dev auth
- L3206 - [sync] Checkout subscription & permission refresh
- L3216 - Modals & Handlers
- L3284 - PDF intake (upload, test PDF, hashing)
- L3292 - Toolbar tool buttons
- L3492 - Tool sidebar buttons & legend overlay
- L3583 - Add Line Type modal
- L3666 - Line color & sidebar handlers
- L3875 - Polyline modal & drawing
- L3918 - Zoom bar & page navigation
- L3944 - Export canvas JSON
- L3960 - PDF download helpers
- L3969 - View-link URL helpers & show-highlights/notes
- L4041 - Custom icon upload handler
- L4051 - Export & report dropdown menus
- L4138 - Sidebar drawer toggles
- L4149 - Mobile actions burger menu pointer & header logo
- L4161 - User Activity pointer (format.js + features/user-activity.js)
- L4173 - My Settings pointer (features/my-settings.js)
- L4198 - Auth & settings entry buttons
  - L4257 - Project Settings checkout & Save Status bell
  - L4349 - [sync] Checkout expired recovery
  - L4405 - [sync] Turn In
  - L4514 - Share modal pointer & copy-project openers
  - L4545 - Settings menu actions
  - L4566 - Auth sign-in form
  - L4591 - Save Project modal
  - L4604 - Checkout expired recovery modal wiring
  - L4709 - Last-session restore prompt
  - L4716 - Canvas Repair modal wiring
- L4903 - Canvas Event Handlers
- L5355 - Event Binding
- L5365 - Aim loupe (mobile press-hold precise placement)
- L5515 - Zoom transform preview & commit
- L5594 - Canvas mouse, wheel & touch handlers
- L6346 - Global dropdown dismissal & keyboard hotkeys
- L6688 - [sync] Manual save to cloud
- L6698 - [sync] Auto-save
- L6705 - [sync] Local backup (IndexedDB takeoff state)
- L6838 - [sync] Checkout keep-alive
- L6852 - App feature registry
- L7162 - View-only mode
- L7168 - Init / boot
