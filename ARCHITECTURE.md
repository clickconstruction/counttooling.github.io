...
| Save before turn in | `saveBeforeTurnInModal` — when local PDF not uploaded and user clicks Save and Turn In |
...
| Custom icon upload | `customIconUploadInput`, `customIconTipsModal`, `getUserCustomIcons`, `saveUserCustomIcons`, `parseUploadedSvg`; Custom Icons grid has + Upload cell; click "Custom Icons" label for tips |
...
- **Save before turn in modal** — When user has local PDF not yet uploaded and clicks Save and Turn In: `saveBeforeTurnInModal` with Cancel, Turn In without PDF, or Save and Turn In (opens save modal, then turns in after save)
