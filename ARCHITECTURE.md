...
- L374 - State
- L615 - [sync] Sync recovery & client recycle
- L1037 - [sync] Global force reload
- L1160 - [sync] Save Status log & envelope
- L1311 - [sync] Dirty tracking & local session reset
- L1526 - [sync] Checkout probe, hashing & PDF cache
- L1773 - Math & Format Helpers
- L2561 - Coordinate Helpers
- L2573 - PDF Rendering
- L3928 - UI Render Functions
- L5058 - Inline rename & polyline edit mode
- L5172 - Item detail & properties modals
- L5510 - Toasts & line color picker
- L5658 - Airboard cloud sync
- L5691 - Supabase RPC & presence heartbeat
- L5731 - User activity / event telemetry
- L5774 - Supabase auth & dev auth
- L5903 - [sync] Checkout subscription & permission refresh
- L6081 - Modals & Handlers
- L6154 - PDF intake (upload, test PDF, hashing)
- L6479 - Toolbar tool buttons
- L6587 - Tool sidebar buttons & legend overlay
- L6701 - Add Line Type modal
- L6771 - Line color & sidebar handlers
- L6915 - Polyline modal & drawing
- L6946 - Zoom bar & page navigation
- L6974 - Canvas layers
- L7177 - PDF download helpers & PipeTooling menu
- L7253 - Copy summaries (PipeTooling / Email)
- L7455 - Import-canvas-after-PDF & Clear Page modals
- L7631 - Download current page
- L7879 - Zone & page-action modal handlers
- L7983 - Mobile actions burger menu (right-side drawer)
- L8111 - User activity time formatting
- L8269 - User Activity modal (admin)
- L8337 - My Settings modal
- L8368 - Auth & settings entry buttons
  - L8413 - Project Settings checkout & Save Status bell
  - L8536 - [sync] Checkout expired recovery
  - L8790 - [sync] Turn In
  - L9303 - Share project & view links
  - L9523 - Cloud project hydrate / copy / fork
  - L9719 - Settings menu actions & Airboard sync
  - L9798 - My Settings password & Auth sign-in
  - L9851 - Save Project modal
  - L10054 - Copy project modal
  - L10078 - Checkout expired recovery modal wiring
  - L10162 - Save-before-load modal
  - L10237 - Last-session restore prompt
  - L10316 - User Activity filters & view toggle
- L10504 - Canvas Event Handlers
- L10876 - Event Binding
- L10879 - Aim loupe (mobile press-hold precise placement)
- L11893 - [sync] Manual save to cloud
- L12518 - [sync] Auto-save
- L12815 - [sync] Local backup (IndexedDB takeoff state)
- L13045 - [sync] Checkout keep-alive
- L13089 - App feature registry
- L13235 - View-only mode
- L13436 - Init / boot
...
`rg "SECTION: \[sync\]" app.js`. In logical (not file) order:
...
- **Measure tool** (`D`) — two-click distance; toast uses the enclosing Scale
  ...
  view mode.
- **Viewer scale status + temporary scale** — view-link viewers see the page's
  scale status on the (no longer viewer-hidden) Set Scale buttons and the desktop
  `#sidebarScaleDisplay`, and may run the full Set Scale flow (`S`, two-point,
  presets, custom) to set a **temporary local-only** scale so Measure reads real
  units. Inherently unsaveable (`markProjectDirty`/`performAutoSave` are
  viewer-inert); `noteViewerTempScale(pageIdx)` stamps `scale.temp = true` (labels
  render "… · temp") and remembers it per link in localStorage
  `view:scale:<token>` (a page-index → scale map); `applyViewerTempScales()`
  restores it in `initViewOnlyMode` **only for pages with no owner scale** — the
  owner's scale always wins. The viewer tool whitelists (updateUI reset,
  ...
  sessions have nothing recoverable, and the temp scale must not leak into
  backups keyed by the owner's project id). Both helpers are published on `App`
  (the apply sites in [features/scale.js](features/scale.js) call
  `App.noteViewerTempScale` defensively). Regression:
  [viewer-temp-scale.spec.js](viewer-temp-scale.spec.js).
