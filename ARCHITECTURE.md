...
- L378 - State
- L590 - [sync] Sync recovery & client recycle
- L706 - [sync] Global force reload
- L796 - [sync] Save Status log & envelope
- L941 - [sync] Dirty tracking & local session reset
- L947 - Undo/redo stacks
- L1133 - [sync] Checkout probe, hashing & PDF cache
- L1309 - Math & Format Helpers
- L2098 - Coordinate Helpers
- L2106 - PDF render bitmap cache
- L2265 - PDF Rendering
- L3705 - UI Render Functions
- L4852 - Inline rename & polyline edit mode
- L4966 - Modal primitives (showModal / hideModal)
- L4985 - Toasts & line color picker
- L5133 - Airboard cloud sync
- L5166 - Supabase RPC & presence heartbeat
- L5206 - User activity / event telemetry
- L5249 - Supabase auth & dev auth
- L5378 - [sync] Checkout subscription & permission refresh
- L5388 - Modals & Handlers
- L5461 - PDF intake (upload, test PDF, hashing)
- L5789 - Toolbar tool buttons
- L5897 - Tool sidebar buttons & legend overlay
- L6011 - Add Line Type modal
- L6081 - Line color & sidebar handlers
- L6225 - Polyline modal & drawing
- L6256 - Zoom bar & page navigation
- L6282 - Export canvas JSON
- L6298 - PDF download helpers
- L6351 - View-link URL helpers & show-highlights/notes
- L6423 - Custom icon upload handler
- L6548 - Export & report dropdown menus
- L6638 - Sidebar drawer toggles
- L6649 - Mobile actions burger menu pointer & header logo
- L6661 - User Activity pointer (format.js + features/user-activity.js)
- L6673 - My Settings pointer (features/my-settings.js)
- L6696 - Auth & settings entry buttons
  - L6741 - Project Settings checkout & Save Status bell
  - L6842 - [sync] Checkout expired recovery
  - L6898 - [sync] Turn In
  - L7177 - Share modal pointer & copy-project openers
  - L7220 - Cloud project hydrate / copy / fork
  - L7417 - Settings menu actions
  - L7438 - Auth sign-in form
  - L7462 - Save Project modal
  - L7665 - Copy project modal
  - L7689 - Checkout expired recovery modal wiring
  - L7773 - Save-before-load modal
  - L7848 - Last-session restore prompt
  - L7927 - Canvas Repair modal wiring
- L8074 - Canvas Event Handlers
- L8446 - Event Binding
- L8456 - Aim loupe (mobile press-hold precise placement)
- L8594 - Zoom transform preview & commit
- L8630 - Canvas mouse, wheel & touch handlers
- L9251 - Global dropdown dismissal & keyboard hotkeys
- L9479 - [sync] Manual save to cloud
- L10104 - [sync] Auto-save
- L10400 - [sync] Local backup (IndexedDB takeoff state)
- L10542 - [sync] Checkout keep-alive
- L10556 - App feature registry
- L10730 - View-only mode
- L11011 - Init / boot
...
Status log core + the dirty core, Stage 3 moved the storage ring —
the checkout probe, hashing, the takeoff-backup wrappers, and the local
backup writer — Stage 4 moved the client-resilience layer — the
recovery/client probes, the client recycle, and the raw-fetch fallbacks —
and Stage 5 moved the checkout-UX domain — the realtime checkout
subscription + `refreshProjectPermissions`, the expired-recovery core
(`reCheckOutAfterExpiry`, the silent auto-recheckout ladder,
`handleBackgroundCheckoutExpired`), and the Turn In core (`doTurnIn`),
while the recovery/Turn-In modal wiring and result-handling UX stayed in
app.js (the engine now OWNS `saveStatusLog`, `dirtyGeneration`,
`dirtyStartedAt`, the backup in-flight/warn/stamp state, the backup
debounce, the probe/recycle guards + wedge stamp, the checkout channel +
reconnect state, the auto-recheckout rate limits, and the Turn In
in-flight guard) — the `[sync]` markers remain in app.js heading
same-named thin wrappers, so the grep still finds the whole
