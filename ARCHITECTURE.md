...
Current first-party line counts (`wc -l`, 2026-08-14 — the **numbers and this
...
|------|------:|------------------|
| [app.js](app.js) | 6,809 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
...
| [app/index.html](app/index.html) | 2,603 | The shell: HTML structure + every modal, no inline JS. Flat markup with no build step to split it; grows roughly linearly with modal count. Leave. |
| [styles.css](styles.css) | 1,521 | All CSS, token-organized. Leave. |
...
| [report.js](report.js) | 559 | Self-contained report builder with a frozen `window.*` contract. Leave. |
| `features/*.js` (59 files) | 13,382 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
...
| [report.js](report.js) | Loads after app.js. Print report, Summary, `getPipeToolingSummary(options)`, `getEmailTextSummary(options)` (both accept `{ pageIndices, getAnnotations }`); `escapeHtml`; consumes globals exposed by app.js via `window.*`. Its `window.*` attachment is guarded by `typeof window` and it has a guarded CommonJS export footer (`escapeHtml`, `pickScaleForLineType`, `orderGroupIds`, `isUntaggedGroupId`, `collectSummaries`) — both inert in the browser — so those helpers can be `require()`d by [report.test.js](report.test.js). Since T1-05 the summary records carry the ft/px split (`runsFt`/`runsPx`, `lengthFt`/`lengthPx`, `pagesFt`/`pagesPx`) and every rollup surface (headline, group totals, summary rows, both text summaries) emits separate ft and px rows — px never summed under a ft label |
| [report.test.js](report.test.js) | Node `node:test` unit tests for [report.js](report.js)'s pure helpers — `escapeHtml` (null/undefined → `''`, entity escaping, `&`-first ordering, `String()` coercion) and `pickScaleForLineType` (preferred-unit selection via a `global.state` stub), `orderGroupIds`/`isUntaggedGroupId`, and the `collectSummaries` ft/px bucketing (stubbed `getLineLengthSplitForTotals`; asserts the summed `lengthReal` key is gone); run with `npm run test:unit` |
| [save-utils.js](save-utils.js) | Pure helpers for the save/sync layer — `isTransientSaveError` (which save/turn-in errors merit one retry), `getProjectCounts` (counter/line totals over a project `data` object, both legacy `annotations` and `canvases` shapes), plus the pure-mined set: `serializeSaveError` (the **deduped** error serializer that replaced app.js's near-identical `serializeSaveErrorForEvent` + `saveDebugSerializeError`), `formatSaveStatusErrDetail`, `backoffDelayMs` (auto-save backoff level for a failure count), `computeClockOffsetMs` (server/local skew from an RPC `server_now`), `percentile` (p95 of latency samples), and `pickBootRestoreCandidate` (the boot-time restore picker over the `'local'` + held takeoff-backup records: held preferred, newer wins when both qualify, data-only fallback with `promptable:false`). Classic `<script src>` loaded before the IIFE; no `state`/DOM dependency — app.js keeps the state-coupled callers (`updateServerClockFromRpc`, the backoff line, `recordAutosaveLatency`) that delegate to these. Guarded CommonJS export footer so the helpers can be `require()`d by [save-utils.test.js](save-utils.test.js) |
...
| [annotation-model.js](annotation-model.js) | **The canvas/annotation data model** (Tier-2 item 7) — exports `createAnnotationModel(ctx)` + `createUndoStack(ctx)`, the same seam recipe as the save engine. Classic `<script src>` loaded after [geometry.js](geometry.js) + [icons.js](icons.js) (reads `bakeFramesMatch`/`rotatePoint90CW`/`pointInRect`/`CIRCLE_PATH` by bare name) and before [save-engine.js](save-engine.js); app.js instantiates both once and keeps same-named thin wrappers so call sites, the App registry, and the feature-file contracts stay frozen. The model owns: `makeAnnotations` (the canonical shape), canvas-layer accessors (`getPageCanvases`/`getActiveCanvas`/`getActiveAnnotations`/`ensureActiveCanvas`/`getMergedAnnotationsForPage`/`mergeAnnotations`/`migratePageToCanvases`), has-any checks, backup↔proj format conversion, bake-frame stamp/verify, the backup/data appliers (`applyTakeoffBackupToState`/`applyPageAnnotationsFromData`), orphan reconcile, the **palette relink pair** (`planPaletteRelink`/`applyPaletteRelink` — Load-from-Cloud's T1-09 name-match relink: maps current→incoming palette ids by trimmed case-insensitive name (first incoming match wins, identity mappings dropped), counts relinked vs orphaned placed marks for the confirm copy, then rewrites `counterMarkers` keys (single-pass rebuild, merging on collision) and quickLine/polyline `lineTypeId`s; reconcile stays the byte-identical backstop), the **rect-select operations** (`countItemsInRect`, `collectItemsToDeleteInRect`, `deleteCollectedItems` — the Delete Area splice core with its load-bearing descending-index order; app.js's `performDeleteZone` keeps the undo/dirty/re-render choreography, ctx supplies `getLineRealWorldLengthFeet`), the **page-rotation math** (`rotateAnnotations`/`applyRotationDeltaToAnnotations` — node-tested 4×90° round trips), and `deepCopyAnnotations`. Module-scope pure helper `dedupePaletteById(list)` (exported + on `App`): collapses same-id palette entries (first position, last fields — newest rename wins); called at the top of `reconcileOrphanedCountersAndLineTypes` (which runs at every palette intake) and by `fetchUserAirboard`/`saveUserAirboard`, so duplicate-id corruption (the Wendi FD bug) self-heals on open and on artboard round-trips. `createUndoStack` owns the undo/redo snapshot stacks (pages/counters/lineTypes/groups/rooms). Guarded CommonJS footer so [annotation-model.test.js](annotation-model.test.js) can `require()` it |
...
| [features/scale.js](features/scale.js) | Thirteenth feature-file split (`window.App` registry pilot #13) — the **Scale modal** (`#scaleModal`), opened by the Set Scale buttons / `S` hotkey and reused for per-page scale, scale-zone create, and scale-zone edit. `updateScalePlaceholder` + `openScaleModal` + `resetScaleModalZoneMode` + `applyScaleObjectToZoneOrPage` + `showScaleTab`, plus the `#setScale`/`#setScaleSidebar` openers and the `#scaleModalTabs`/`#scaleUnit`/`#scaleSelectOnPdf`/`#scalePresetsCancel`/`#scaleCustomApply`/`#scaleCancel`/`#scaleSet` handlers (which had lived down in the Counter-modal region). Its own IIFE loaded **after** [app.js](app.js); reads shared `state`/helpers from `window.App` at call time, registers `App.openScaleModal` + `App.resetScaleModalZoneMode`, binds everything at load. **First split to route geometry.js globals + `SCALE_*` constants through the registry** — six new publish-only deps `SCALE_MODES`/`SCALE_PRESETS`/`ptDist`/`parseFraction`/`parseRealWorldLength`/`getActiveAnnotations` (stay in app.js, read via `App.*` so the `features/*.js` group's browser-only globals don't trip `no-undef`); `state`/`showModal`/`hideModal`/`updateUI`/`renderPdf`/`pushUndoSnapshot`/`markProjectDirty`/`uid`/`ensureActiveCanvas`/`showToast`/`TOOL` were already on `App`. The modal doubles as the scale-zone create/edit dialog (`scaleModalApplyTarget === 'zone'`), so `applyScaleObjectToZoneOrPage` moves with it; the four `openScaleModal` callers (canvas two-point finish + scale-zone context-menu Edit) and the Escape-key `resetScaleModalZoneMode` branch keep their zone-entry state/DOM setup inline and reach the modal via `App.*`. The toolbar tool buttons (`#measureBtn`/`#moveBtn`/`#quickLine`/`#undoBtn`/`#redoBtn`/`#polylineBtn`/`#highlightBtn`/`#multiplyZoneBtn`/`#scaleZoneBtn`/`#deleteZoneBtn`) that shared the grab-bag stay in app.js. **Renamed** the section marker `// SECTION: Scale modal` → `// SECTION: Toolbar tool buttons` (TOC stays 49). **T1-04**: a preset/custom **page-scale** apply with a sheet correction in play hands off straight into the two-point verify (`handOffToVerify` → `startScaleCheck` + a coaching toast naming the assumed sheet; escapable — Esc / Keep current scale keep the applied scale), and the file fires its first telemetry: `App.logUserEvent('scale_set', …)` on every page-scale apply (`logScaleSet` — method preset/custom/two-point/use-measured, correctionFactor, verifyHandoff) + `scale_verify` (deltaPct) in the `#scaleCheckBtn` handler. **T1-07**: zone preset/custom applies inherit the page scale's **stamped** sheet correction (product decision 2026-08-09) via `withZoneSheetCorrection` — reads `page.scale.correctionFactor`/`sheetSize` (the banner/picker stays page-only), folds the factor into the zone's `pixelsPerUnit`, stamps `correctionFactor`+`sheetSize`+the label suffix, and the zone dialog's info line discloses it ("corrected as if printed on …"); two-point zone calibration stays raw, and `applyScaleObjectToZoneOrPage(scaleObj, method)` fires the zone-flavor `scale_set` (`target:'zone'`, method preset/custom/two-point, edit flag) |
| [scale.spec.js](scale.spec.js) | Playwright regression for pilot #13 — uploads `test-2pages.pdf`, asserts `window.App.openScaleModal` + `resetScaleModalZoneMode` are functions and `Array.isArray(App.SCALE_PRESETS)`, opens via the registry, clicks a preset and asserts `state.pages[currentPage].scale` was set + the modal closed, reopens and exercises `#scaleCustomApply` with a valid fraction + feet asserting the computed `pixelsPerUnit` + closed modal; asserts no console / page errors. T1-04 cases (sheet analysis stubbed in-page to put a correction in play): corrected preset apply arms the verify + Esc keeps the applied scale; corrected custom apply hands off while a standard sheet keeps the plain toast; and the full verify loop (two canvas clicks → check panel → Check delta → Keep current scale preserves / Use measured recalibrates). T1-07 zone-correction cases (same stub; zone mode entered via the app.js state seam): corrected zone preset stores `18 × factor` with the ` · ANSI D` suffix, the dialog discloses the inheritance and never shows the sheet picker; corrected custom apply matches while a two-point zone recalibration stays raw (no `correctionFactor`); an uncorrected page keeps the raw preset; edit-mode re-apply applies the factor once (no compounding); `npx playwright test scale.spec.js` |
| [scale-modal-clamp.spec.js](scale-modal-clamp.spec.js) | Playwright regression for the T1-03 viewport clamp — the Set Scale card stays within short viewports (≤900px), tab row and Apply reachable, wheel scrolls the card. `npx playwright test scale-modal-clamp.spec.js` |
...
| [features/output.js](features/output.js) | Twenty-sixth feature-file split (`window.App` registry pilot #26) — the **output-actions cluster** (the "Output" features): **Copy to PipeTooling** (`#forPipeTooling` dropdown toggle + `doCopyPipeTooling` with the view-link footer + the prefetched export view-link cache `exportViewLinkUrl`/`exportViewLinkProjectId` + `canExportViewLink`/`prefetchExportViewLink`, gated by the **pre-copy scale check** `collectUnscaledLinePages`/`runGatedCopy` + `#toolingScaleCheckModal` — since T1-05 the gate fronts BOTH copy surfaces and the `pendingToolingExport` stash carries the copy function (`doCopy`) + dropdown `mode`, with the `App.onToolingScaleCheckHidden` hide callback; successful copies log `copy_summary` and gate hits log `unscaled_ft_block`), **Copy Summary** (`#copySummaryText` dropdown + `doCopyEmailSummary`, gated through the same `runGatedCopy`), and **Download current page** (`downloadCurrentPageAsPdf` + `#downloadCurrentPageBtn` + its mode menu). No entry points registered — the bindings move with their DOM elements, so the mobile burger menu's dispatched clicks keep working untouched; the one registration is the `App.onViewLinkRevoked()` callback (the Share modal's revoke clears the private cache through it). Two new publish-only deps `SUPABASE_ENABLED`/`getOrCreateViewLinkUrl` (the view-link minting **stays** in app.js — the header Share button uses it too — under the renamed marker `// SECTION: View-link URL helpers & show-highlights/notes`); reuses `state`/`getSupabase()`/`showToast`/`showModal`/`hideModal`/`sanitizeForFilename`/`ensureActiveCanvas`/`getPageCanvases`/`renderAnnotationsToContext`/`makeAnnotations`/`logUserEvent` + the `window.*` report fns. The `downloadProjectPdf`/`downloadPdfBuffer` helpers and the header export/report dropdowns stay in app.js (markers renamed `// SECTION: PDF download helpers` and `// SECTION: Export & report dropdown menus`) |
| [output.spec.js](output.spec.js) | Playwright regression for pilot #26 — with clipboard permissions granted: the Copy Summary option writes the email summary to the clipboard + shows the copied modal; the Copy to PipeTooling option writes the tab-delimited summary and shows the "save to include a view link" toast (cloud enabled, no cloud project → no footer); the Download button opens its mode menu on a multi-page project and the this-canvas option yields a real download named `takeoff-page1_*.pdf`; `App.onViewLinkRevoked` is registered. A second test pins the pre-export scale check: an unscaled line page flags in `#toolingScaleCheckModal` (counter-only pages don't), Cancel drops the export, Export anyway copies with the px unit, Set scale jumps to the flagged page and opens the Set Scale modal, and a scale zone around the line passes the check without a page scale. A third test (T1-05) pins that the same gate fronts Copy Summary: the modal opens with nothing written to the clipboard, Cancel drops the pending copy, and Export anyway copies the email summary with its honest `px of` row. Asserts no console / page errors; `npx playwright test output.spec.js` |
| [copy-tooling-feet.spec.js](copy-tooling-feet.spec.js) | Playwright regression owning the copy-surface length convention — a 240-pdf-pt line reads "10.00 ft" identically in the Line Types badge, `getPipeToolingSummary()` and `getEmailTextSummary()` on both inch- and foot-scaled pages (always decimal feet, never the scale's native unit); a second test (T1-05) pins the mixed scaled/unscaled split: one `ft of` row (10.00, page 1) + one `px of` row (367, page 2), the summed "377" appearing on NO surface, both email bullets (the px one flagged "no scale set"), and the sidebar badge + `#statusTotals` reading `10.00 ft + 367 px`. `npx playwright test copy-tooling-feet.spec.js` |
| [scale-gate-page-switch.spec.js](scale-gate-page-switch.spec.js) | Playwright regression for T1-05 Part 3 — the Set-Scale gate re-fires on page switch (`recheckScaleGateOnPageSwitch` in app.js, called from `renderPdf`, the sink for every nav entry point): with Quick Line armed, flipping onto an unscaled page shows the existing "Set Scale first" toast, drops the tool to Move (`state.tool === 0`), and subsequent canvas clicks place nothing; flipping between two scaled pages keeps the armed tool with no toast. `npx playwright test scale-gate-page-switch.spec.js` |
...
| [features/restore-last-session.js](features/restore-last-session.js) | The **last-session restore flow** (Tier-2 split) — `doRestoreLastProject` (full session rebuild from a cloud project row or IDB takeoff backup; PDF ladder: IDB blob → cached blob → signed-URL render → storage download with background re-cache), the `#lastSessionRestoreModal` Keep/Discard handlers (Keep defers the Supabase fetch to click time; offline falls back to the IDB backup; inaccessible projects are cleaned up), and the private `pendingRestore`. Boot (app.js init) detects the candidate — signed-out included (T1-01): a promptable `'local'` backup is keyed aside to `TAKEOFF_BACKUP_HELD_ID` and offered regardless of session — and hands it over via `App.openLastSessionRestorePrompt({proj,cachedBlob,heldBackup} | {cloudLast})`; the local Keep uses `heldBackup` directly (no poisonable store re-read) and consumes the held record on Keep/Discard. Local (`id:'local'`) sessions land `isViewer=false` / `canCheckOut=false` — no checkout exists, and the viewer guard would otherwise silence future backups. `App.isRestorePromptPending` (getter over `pendingRestore`) is polled by the save engine's clobber guard: backup writes hold while the prompt is unresolved. `resetLocalSessionState` clears the flag via the defensive `App.onLastSessionRestoreReset`. Telemetry: `restore_keep` (`{source, markers, ms}`). idb primitives + `pdfjsLib` are classic-script globals; everything else via `App.*` at call time. Regression: [restore-last-session.spec.js](restore-last-session.spec.js) |
...
| [features/burger-menu.js](features/burger-menu.js) | Thirtieth feature-file split (`window.App` registry pilot #30) — the **mobile right-side burger drawer** (`closeBurgerMenu`/`updateBurgerMenu` + the `#headerBurger`/`#rightMenuBackdrop` bindings) and the **desktop header-overflow compact mode** (`updateHeaderCollapsed`/`scheduleHeaderCollapseCheck` + the resize listener + the load-time initial check), moved together because they are one consolidation feature sharing `closeBurgerMenu`. Registers `App.updateBurgerMenu` + `App.scheduleHeaderCollapseCheck`, which `updateUI` invokes **defensively** (`App.fn && App.fn()`) at its tail — a boot-time updateUI before this file loads is a harmless no-op (the load-time check + on-open rebuild cover it). Drawer rows dispatch the click of their CSS-hidden source control and clone its `<svg>`, so no deeper app.js functions are referenced; deps are just `state` + `SUPABASE_ENABLED` (both pre-published — zero new deps). Regressions: the pre-existing [mobile-burger-menu.spec.js](mobile-burger-menu.spec.js) + [header-overflow.spec.js](header-overflow.spec.js), which were written for this exact feature. **Since [features/header-more.js](features/header-more.js) (2026-08-14)**: `scheduleHeaderCollapseCheck` delegates to `App.scheduleHeaderMoreCheck` when present (the ⋯ overflow owns the one-pass resize pipeline) and `updateHeaderCollapsed` is exported on `App` for the orchestrator to run synchronously against the ⋯-reduced row |
| [features/header-more.js](features/header-more.js) | The **header "⋯ More tools" overflow** (desktop; field feedback 2026-08-14 — the tools row previously overflowed into an invisible-scrollbar scroll, so tail tools looked cut off). The toolbar is priority-reordered (Set Scale, Move, Counter, Quick Line, Polyline, Snap-45, Measure, Highlight first); at >768px the fixed low-frequency group (Polyline, Highlight, Multiply Zone, Scale Zone, Room Sizer, Delete Area, Note, Legend, Grid + the group divider) UNCONDITIONALLY lives behind `#headerMoreBtn`'s dropdown — rows show icon + NAME + hotkey (the menu doubles as hotkey education), click through to the REAL buttons, and forward right-clicks so tool-context-menu settings still open. Viewer-hidden sources skip their rows. The ⋯ takes the shared gold `.active` when the current TOOL lives in the menu (Legend/Grid are overlay toggles — their row shows toggle state but is excluded from the indicator). **Owns the one-pass resize pipeline**: engage `body.header-more` (no overflow measure since 2026-08-15 — even a roomy full row read as clutter, so the tuck is unconditional on desktop) → run `App.updateHeaderCollapsed()` synchronously against the reduced row. `updateUI` calls `App.onHeaderMoreSync` defensively. Regression: [header-more.spec.js](header-more.spec.js) + the adapted [header-overflow.spec.js](header-overflow.spec.js) |
...
| [features/my-settings.js](features/my-settings.js) | Thirty-second feature-file split (`window.App` registry pilot #32) — the **My Settings modal** (`#mySettingsModal`), the surface pilot #20 deliberately deferred: `openMySettings` (signed-out falls through to the auth modal via a dispatched `#authBtn` click), the **Artboard** rows (Save/Load via the newly-published engine helpers `App.saveUserAirboard`/`App.fetchUserAirboard`, Export to `artboard-backup.json`, Clear-with-defaults using the newly-published `App.PLUMBING_DEFAULTS`/`App.LINE_DEFAULTS`; the Load row is the T1-09 flow fetch→`App.planPaletteRelink`→confirm-with-real-numbers→`App.pushUndoSnapshot`→palette apply→`App.applyPaletteRelink`→`App.reconcileOrphanedCountersAndLineTypes`→`App.markProjectDirty`→`artboard_load` `App.logUserEvent`, so a mid-bid load re-links placed marks by name instead of orphaning them), the change-password form (`supabase.auth.updateUser` via `App.getSupabase()`), sign-out, close, and the admin Manage-Users/Manage-User/All-Users openers (feature-to-feature: `App.openManageUserModal`/`App.openAllUsersModal` + a dispatched `#manageUsersBtn` click into [features/user-admin.js](features/user-admin.js), whose `#mySettingsMyActivity` binding was already there). Registers `App.openMySettings`; the three openers (`#authBtn` signed-in path, `#sidebarLogoUser`, `#statusBarAuth`) stay in app.js as deferred `App.*` calls. The Airboard engine (`fetchUserAirboard`/`saveUserAirboard`) and the auth sign-in form stay in app.js (markers renamed `// SECTION: My Settings pointer` / `// SECTION: Settings menu actions` / `// SECTION: Auth sign-in form`) |
| [my-settings.spec.js](my-settings.spec.js) | Playwright regression for pilot #32 — always-run: `App.openMySettings` registered; signed-out open falls through to the auth modal; Export artboard yields a real `artboard-backup.json` download; Clear artboard empties the palette + resets active tool state; the close binding hides a force-shown modal; the stubbed-fetch Load tests pin the apply wiring (custom icons + `replace:true` Quick Key seed) and the T1-09 relink — placed marks re-key to the loaded ids by name, unmatched marks surface as an "Unknown" row, the confirm states the real numbers, and Ctrl+Z restores the pre-load palette + marker keys. The airboard cloud round-trip and password change stay cloud-gated per convention. Asserts no console / page errors; `npx playwright test my-settings.spec.js` |
| [features/palette-insights.js](features/palette-insights.js) | **Palette Insights** (`#paletteInsightsModal`, opened from My Settings → Artboard → **Analyze My Usage**): cross-project counter / line-type usage via the `user_palette_usage` RPC (server-side aggregation — the client never downloads whole project JSONB blobs), ranked unadded-first, then by project breadth (breadth beats raw volume — one huge bid shouldn't dominate), then placements; the min-projects segmented filter (localStorage-persisted) filters both lists AND drives "Add all shown". One-click **ADDITIVE** adds to the cloud Artboard — a narrow fetch-merge-upsert of only the counters/line_types columns, deliberately NOT `App.saveUserAirboard` (the wholesale save, which would replace the Artboard with the current project's palette). Identity is case-insensitive name matching (counter/line-type ids are `uid()`-scoped per project) **plus an id guard** (the Wendi FD fix, 2026-08-13): an incoming row whose `item_id` already exists in the artboard/project is a RENAME generation of that entry, so both add paths update it in place instead of appending — appending made every rename claim the same `counterMarkers[id]` array, counting one placed mark once per generation. Both merge inputs are also passed through `App.dedupePaletteById`. Adds also land immediately in the open project. Regression: [palette-insights.spec.js](palette-insights.spec.js) |
...
| [features/chain.js](features/chain.js) | The **Chain tool** (`TOOL.CHAIN`, hotkey T) — one click per fixture, connecting runs ride along: `commitChainPoint(pdf)` drops a counter marker every click and, from the second click on, a quick line back to the previous counter in the SAME undo snapshot. Placements are ordinary counter markers + quick lines (no new persisted shapes). Owns the floating two-column palette panel (`#chainPanel`: searchable Counters / Line-type columns; row clicks write `state.activeCounterType` / `state.activeLineTypeId` DIRECTLY — not via `setActive*`, whose side effect is switching the tool) and the `onChainToolSync` core→feature callback `updateUI` invokes (panel show/hide + row re-render; search inputs live outside the lists so typing focus is never stolen). Anchor `state.chainStart` `{x,y,page}` — page stamp invalidates it across page switches; Esc/tool-switch/Move resets live in app.js beside the quick-line siblings, as do the TOOL.CHAIN click branch, the rubber-band preview (45°-snap aware), and the scale gate (button + page-switch `gated` map). Publish-only deps: `isPointInPageBounds`, `showOutOfBoundsToast`, `snapLineToAngle`, `clampPointToPageBounds`, `snapToGrid`, `logCounterMarkerAddedEvent`, `logLineAddedEvent`, `collapsePagesSectionForPlacing`. Registers `commitChainPoint` + `onChainToolSync`. Regression: [chain.spec.js](chain.spec.js) (4 tests) |
| [features/hotkey-peek.js](features/hotkey-peek.js) | **Hotkey peek** — hold Cmd (Meta; Alt as the Windows/Linux alias) for ~1.5s without pressing another key and `body.hotkey-peek` reveals a `<kbd class="hk-badge">` chip on every visible hotkey-carrying control; release, or any focus loss (blur / `visibilitychange` — Cmd+Tab never delivers the Meta keyup), hides them, and a second key during the hold cancels the pending peek (a Cmd+S-style combo is a shortcut, not a question). Badges are STAMPED lazily on first peek from `App.HOTKEYS` (the hotkeys.js single source) — non-bespoke `btnId` entries map directly, runner entries via `RUNNER_BADGE_TARGETS` (moveReset→moveBtn, toggleSnap→lineTypeSnapToHVHeaderBtn, rotatePage→rotatePage), each also stamping its `<id>Sidebar` twin — so a new hotkey grows its badge automatically. Purely a visual layer: never handles the hotkeys themselves. Zero registry deps beyond `App.HOTKEYS`; exposes the `__hotkeyPeekState` spec seam. Regression: [hotkey-peek.spec.js](hotkey-peek.spec.js) (3 tests) |
| [features/child-counts.js](features/child-counts.js) | **Child counts** — words-only quantities riding a parent counter or line type (`item.childCounts: [{ name, qty, per, ftInterval }]`, per ∈ `'count'`\|`'run'`\|`'ft'`). DERIVED at tally time, never marks on the sheet: `getChildCountTotals({ pageIndices?, getAnnotations? })` → `byGroup[gid][kind][parentId]` rows `{ name, qty, per, ftInterval, total, excludedPxRuns }` — per-count/per-run totals multiply through zones; per-ft = `ceil(rawFeet / N) × zone × qty` PER RUN (zoned split.feet divided back to raw first — a ×10 zone is 10 identical runs each rounding its own supports); per-ft on unscaled (px) runs is EXCLUDED + flagged (T1-05 discipline). The rule rides save/load/export-import/Artboard for free (palettes serialize wholesale; relink is name-safe). Consumers: [features/summary-list.js](features/summary-list.js) (SEPARATE indented `.summary-child-item` rows per parent per group), [report.js](report.js) via guarded `window.App` lookup (the room-sizer recipe) — report Summary tables (indented ↳ rows + px-exclusion footnote), the email summary (indented ↳ bullets), and Copy to PipeTooling, where child rows are INDENTED two spaces and the same name across parents MERGES into one row under the first parent using it. Also registers `renderChildCountsSection(kind, item)` — the editor section in `#counterLineTypeDetailsModal` (features/item-details.js calls it defensively on open). Regression: [child-counts.spec.js](child-counts.spec.js) (4 tests) |
| [features/view-only.js](features/view-only.js) | The **view-link session**: the email-gated `initViewOnlyMode(viewToken)` boot (fetch via the `get-view-project` Edge Function, offline view-cache fallback, page/annotation hydration, viewer `state` flags), the viewer-scale sharing layer (`shareViewerScale` → the `set-view-scale` Edge Function, with the per-token `view:scale:<token>` localStorage temp fallback via `noteViewerTempScale`/`applyViewerTempScales`), the owner-side `maybeShowViewerScaleNotice` must-clear notice, and the **dead-link failure screen** (`showViewLinkFailure` — the opaque full-screen `#viewLinkDeadScreen`: server-rejected 4xx errors are tagged `viewLinkDead` in `fetchViewProject`, get the "isn't active anymore" copy, and override the offline view-cache fallback so a revoked link truly stops cached revisits; network/5xx failures get the connection copy + a Retry button that just reloads; emits `view_link_dead` via `App.logUserEvent` when a session exists). Registers `App.initViewOnlyMode` (awaited by app.js boot), `App.shareViewerScale` (consumed by app.js AND features/scale.js, guarded), `App.noteViewerTempScale`, `App.applyViewerTempScales` (a `viewer-scale.spec.js` test seam), `App.maybeShowViewerScaleNotice`, `App.cancelViewLinkEmailPrompt` (lets app.js's global Escape handler cancel the email prompt), `App.showViewLinkFailure` (called defensively by app.js's boot catch, toast fallback). Regression: [view-only.spec.js](view-only.spec.js) + [viewer-scale.spec.js](viewer-scale.spec.js) + [rotation-share-roundtrip.spec.js](rotation-share-roundtrip.spec.js). |
| [features/pdf-intake.js](features/pdf-intake.js) | The **PDF upload pipeline**: the `#pdfInput` change handler (size caps, multi-file merge, append mode via Prepare PDF, pending-canvas-load hash matching, the load-annotations prompt, first-upload Prepare handoff), `loadTestPdf` (localhost-only), and `titleFromPdfFilename`. Computes the upload hash once in `handleFreshUpload`, stashes it as the in-memory-only `state.localPdfHash` (signed-out backups carry it), and runs `maybeReapplyLocalBackupMarks` (T1-01/J4): a signed-out same-PDF re-upload re-applies the marks from the on-device backup (held key first, then `'local'`) — hash-verified only, consumed on apply, toast on success. Appending to loaded pages or an open project never renames it — the filename→name assignment only runs on a fresh upload into an empty session — and appends toast `Added N sheets to <project>` (T1-08/J2). Owns the `pendingImportCanvasAfterPdf` / `pendingAddAdditionalPages` flags; registers `App.loadTestPdf`, `App.titleFromPdfFilename`, `App.setPendingAddAdditionalPages`, `App.resetPdfIntakeFlags`. Bidirectional with features/prepare-pdf.js (`App.openPreparePdfModal`) and features/load-project.js (`App.hydrateProjectFromCloudRow`). Regression: [pdf-upload.spec.js](pdf-upload.spec.js) + [add-pdf-pages-canvas-jump.spec.js](add-pdf-pages-canvas-jump.spec.js) (no name-matched spec). |
...
- L176 - State
- L349 - [sync] Sync recovery & client recycle
- L430 - [sync] Global force reload
- L514 - [sync] Save Status log & envelope
- L517 - [sync] Field-error telemetry
- L576 - [sync] Dirty tracking & local session reset
- L582 - Undo/redo stacks
- L713 - [sync] Checkout probe, hashing & PDF cache
- L775 - Math & Format Helpers
- L1188 - Coordinate Helpers
- L1196 - PDF render bitmap cache
- L1250 - Sharp crop tile (deep-zoom sharpening + window-first commits)
- L1261 - PDF Rendering
- L1981 - UI Render Functions
- L2552 - Inline rename & polyline edit mode
- L2666 - Modal primitives (showModal / hideModal)
- L2696 - Toasts & line color picker
- L2750 - Airboard cloud sync
- L2795 - Supabase RPC & presence heartbeat
- L2835 - User activity / event telemetry
- L2878 - Supabase auth & dev auth
- L3010 - [sync] Checkout subscription & permission refresh
- L3020 - Modals & Handlers
- L3088 - PDF intake (upload, test PDF, hashing)
- L3096 - Toolbar tool buttons
- L3231 - Tool sidebar buttons & legend overlay
- L3322 - Add Line Type modal
- L3407 - Line color & sidebar handlers
- L3616 - Polyline modal & drawing
- L3647 - Zoom bar & page navigation
- L3673 - Export canvas JSON
- L3689 - PDF download helpers
- L3698 - View-link URL helpers & show-highlights/notes
- L3770 - Custom icon upload handler
- L3780 - Export & report dropdown menus
- L3867 - Sidebar drawer toggles
- L3878 - Mobile actions burger menu pointer & header logo
- L3890 - User Activity pointer (format.js + features/user-activity.js)
- L3902 - My Settings pointer (features/my-settings.js)
- L3925 - Auth & settings entry buttons
  - L3970 - Project Settings checkout & Save Status bell
  - L4071 - [sync] Checkout expired recovery
  - L4127 - [sync] Turn In
  - L4229 - Share modal pointer & copy-project openers
  - L4260 - Settings menu actions
  - L4281 - Auth sign-in form
  - L4305 - Save Project modal
  - L4318 - Checkout expired recovery modal wiring
  - L4423 - Last-session restore prompt
  - L4430 - Canvas Repair modal wiring
- L4583 - Canvas Event Handlers
- L4983 - Event Binding
- L4993 - Aim loupe (mobile press-hold precise placement)
- L5133 - Zoom transform preview & commit
- L5212 - Canvas mouse, wheel & touch handlers
- L5873 - Global dropdown dismissal & keyboard hotkeys
- L6149 - [sync] Manual save to cloud
- L6159 - [sync] Auto-save
- L6166 - [sync] Local backup (IndexedDB takeoff state)
- L6299 - [sync] Checkout keep-alive
- L6313 - App feature registry
- L6586 - View-only mode
- L6592 - Init / boot
...
| Length tally in feet (per-line) / ft-px split (rollups) | `getLineLengthFeetForTotals` / `getLineLengthSplitForTotals` or `lineLengthFeetForTotals` / `lineLengthSplitForTotals` (line-metrics.js) or `formatFeet` / `formatFeetPx` (geometry.js) |
...
- **Verify-your-scale advisory + check mode** — because a preset/custom scale is an
  *assumption* (and the sheet-size correction a best guess), a persistent **blue**
  ...
  reports the delta. On a corrected preset misreading ≥5%, the reverse lookup
  `sheetMatchingCorrection` (pure, [geometry.js](geometry.js): the sheet whose
  correction factor matches cfNeeded = cfCur × reading/known) names in
  `#scaleCheckSheetHint` the sheet the page really measures like — resolving the
  ARCH B/D aspect-twin ambiguity for the rest of the set
  ...
  scale also fires a brief **post-apply toast** nudging verification — except when a sheet
  correction is in play: that apply flows **straight into this check flow** (T1-04, below).
  `resetScaleCheckMode`
...
- **Sheet-size correction (compressed-PDF fix)** — the architectural presets and
  ...
  **not** → a yellow `#scaleSheetWarning` banner + `#scaleSheetSelect` picker appears,
  ...
  The warning banner/picker is primed on the **page-scale presets tab only**; zone
  preset/custom applies **inherit the page's stamped factor** via
  `withZoneSheetCorrection` (product decision 2026-08-09 — reads
  `page.scale.correctionFactor`/`sheetSize`, so the zone never re-asks; the zone dialog's
  info line discloses it). The two-point "Select on PDF" path — page or zone — is never
  corrected (already ground truth, no `72`). Because the corrected
  scale is still a *guess*, a corrected apply hands off directly into the two-point
  verify check (T1-04): the modal hides, `TOOL.SCALE` arms with `scaleCheckMode` set, and
  the toast reads "Scale set as if printed on `<sheet>` — click both ends of a printed
  dimension to check it (Esc keeps this scale)" — escapable at every stage, so a correct
  guess costs zero extra clicks beyond Esc. Limitation: when a
...
- **Chain tool** (`T`, `TOOL.CHAIN`, [features/chain.js](features/chain.js)) —
  ...
  undo step. Placements are ORDINARY counter markers + quick lines (zero new
  persisted shapes). A floating two-column palette panel (`#chainPanel`,
  searchable Counters | Line types) shows while active; selection writes
  `state.activeCounterType` / `state.activeLineTypeId` directly. Anchor:
  `state.chainStart` `{x, y, page}` (page stamp invalidates across page
  switches). Enter or Esc ends the run (Esc again exits to Move); scale-gated
  like Quick Line; both palette columns end with a "+ New" action row driving
  the real sidebar create buttons (tool stays CHAIN through the create);
  the palette is DRAGGABLE by its title bar (position per device in
  localStorage `chainPanelPos`, viewport-clamped) and CLOSABLE without
  leaving the tool (title-bar ×, or the ladders: Enter = end run -> close;
  Esc = end run -> close -> Move) — while closed, the `#headerChainPair`
  chip (current pair, from the active selections) sits right of the Chain
  button and reopens the palette on click, as do T / the Chain button;
  preview honors 45° snap, grid snap applies to un-anchored placements.
  Regression: [chain.spec.js](chain.spec.js).
...
- **Sidebar usage filter (off / sheet / project)** — the inline **funnel**
  button next to the Counters / Line Types search boxes cycles Off → This
  sheet → This project (sheet scope shows a corner dot, project scope swaps
  in a stacked-sheets glyph — all three states visually distinct; titles
  narrate the state, and each cycle click fires a two-line toast —
  "Filter:" / landed state — via `showFilterScopeToast`, whose
  `showFilterToast` core the two-state Lines toggle reuses). The chosen scope
  persists per device (`counterSidebarFilterScope` /
  `lineTypeSidebarFilterScope` in localStorage, read at boot — design
  recovered from the unlanded bb19fa attempt); hint rows state the reason
  ("N not used on this sheet / in this project — show all");
...
- **Counter/Line Type details modal** — edit pen opens
  ...
  confirms).
- **Child counts** ([features/child-counts.js](features/child-counts.js)) —
  words-only quantities that ride a parent counter or line type into the
  Summary, report, email summary, and Copy to PipeTooling: `2 × connector per
  run`, `2 × ground screw per count`, `1 × strap per 10 ft` (ceil per run;
  unscaled runs excluded + flagged). Defined in the details modal's "Child
  counts" section; derived at tally time — never marks on the sheet; separate
  rows per parent in the Summary, indented + name-merged rows in PipeTooling.
...
- **Groups** — assign counters/lines to a group; `groupAssignModal` + `groupModal`
  (Add/Edit); Show group colors toggle. **Per-project gate**: the Groups UI
  (sidebar `#groupsSection` + the context-menu Assign-to-Group entry) shows only
  when `groupsUiVisible()` — `state.groupsEnabled` (the Project Settings "Use
  groups in this project" toggle, `#settingsUseGroupsBtn`) OR the project already
  has groups (auto-show, toggle locked on). Creating the first group latches the
  flag true so deleting the last group can't hide the section mid-session.
  `groupsEnabled` rides save/load + export/import + the IndexedDB backup and is
  restored by both shared hydrate paths and the copy/load/import intakes.
  Regression: [groups-per-project.spec.js](groups-per-project.spec.js)
...
- **Length tallies: decimal feet, with a strict ft/px split (T1-05).** Every takeoff
  rollup (Line Types sidebar, Lines list, Summary panel, footer totals, Copy to
  PipeTooling, Copy Summary email/text, printable Report, embedded PDF legend)
  accumulates lengths in two buckets via `getLineLengthSplitForTotals` →
  `lineLengthSplitForTotals` + `convertUnitValue`: **feet** for lines with a usable
  effective scale (unit-converted before summing, fixing mixed-unit summation) and
  **raw px** for lines on unscaled pages — and no surface ever adds the buckets
  together (`formatFeetPx`: "12.50 ft", "200 px", "12.50 ft + 200 px"). The exports
  emit separate `ft of` / `px of` rows per line type. Per-line surfaces (count-detail
  drill-down, Lines-list rows, item details) keep `getLineLengthFeetForTotals` /
  `getLineRealWorldLengthFeet` + `formatFeet`. Only the **on-canvas per-line length
  labels** and the **Measure-tool** readout keep feet-inches notation (the
  construction-drawing convention).
...
- **Undo/Redo** — last 50 moves in memory (`UNDO_STACK_SIZE`, constants.js —
  ...
  Ctrl+Shift+Z; cleared on load/switch/viewer. Each successful undo toasts the
  remaining count ("2 undos left", 1s) via the app.js `undo()` wrapper reading
  `undoDepth()`; OS key auto-repeat is ignored (`e.repeat`) so holding Ctrl+Z
  performs exactly one undo per physical press. [undo-toast.spec.js](undo-toast.spec.js).
...
- **View links** — `project_view_links` + `view_link_access_log`; Share modal
  ...
  **revalidates against the server when online** (reusing the cached PDF blob by hash,
  ...
  A dead, revoked, or unreachable link lands on a full-screen plain-language message
  (`showViewLinkFailure`; Retry on network failure) instead of a toast over the empty
  editor, and a server-confirmed revocation overrides the offline cache.
