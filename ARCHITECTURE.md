...
|------|------:|------------------|
| [app.js](app.js) | 6,713 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
...
| [app/index.html](app/index.html) | 2,525 | The shell: HTML structure + every modal, no inline JS. Flat markup with no build step to split it; grows roughly linearly with modal count. Leave. |
| [styles.css](styles.css) | 1,435 | All CSS, token-organized. Leave. |
...
- L3565 - Polyline modal & drawing
- L3596 - Zoom bar & page navigation
- L3622 - Export canvas JSON
- L3638 - PDF download helpers
- L3647 - View-link URL helpers & show-highlights/notes
- L3719 - Custom icon upload handler
- L3729 - Export & report dropdown menus
- L3816 - Sidebar drawer toggles
- L3827 - Mobile actions burger menu pointer & header logo
- L3839 - User Activity pointer (format.js + features/user-activity.js)
- L3851 - My Settings pointer (features/my-settings.js)
- L3874 - Auth & settings entry buttons
  - L3919 - Project Settings checkout & Save Status bell
  - L4020 - [sync] Checkout expired recovery
  - L4076 - [sync] Turn In
  - L4178 - Share modal pointer & copy-project openers
  - L4209 - Settings menu actions
  - L4230 - Auth sign-in form
  - L4254 - Save Project modal
  - L4267 - Checkout expired recovery modal wiring
  - L4372 - Last-session restore prompt
  - L4379 - Canvas Repair modal wiring
- L4532 - Canvas Event Handlers
- L4926 - Event Binding
- L4936 - Aim loupe (mobile press-hold precise placement)
- L5075 - Zoom transform preview & commit
- L5154 - Canvas mouse, wheel & touch handlers
- L5815 - Global dropdown dismissal & keyboard hotkeys
- L6077 - [sync] Manual save to cloud
- L6087 - [sync] Auto-save
- L6094 - [sync] Local backup (IndexedDB takeoff state)
- L6227 - [sync] Checkout keep-alive
- L6241 - App feature registry
- L6502 - View-only mode
- L6508 - Init / boot
...
- **Sidebar usage filter (off / sheet / project)** — the inline button next to
  the Counters / Line Types search boxes cycles Off → This sheet → This project
  (sheet scope shows a corner dot, project scope swaps in a stacked-sheets
  glyph — all three states visually distinct; titles narrate the state, and
  each cycle click fires a three-line toast — "Filter:" / landed state /
  dimmed next-click hint — via `showFilterScopeToast`). The chosen scope
