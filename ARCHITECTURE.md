...
| Toggle switches | `toggle-switch`, `toggle-switch-knob`; used for Show group colors, Counter Settings (Show ring, Solid ring), Save Project Include PDF, Combined PDF/Specific Pages (Bundle highlights, Bundle notes, Include report) |
...
| Last session restore | `lastSessionRestoreModal` or `pendingLastSessionRestore` or `doRestoreLastProject` |
...
| Prepare PDF modal | `preparePdfModal` or `openPreparePdfModal` or `commitPreparePdfToState`; Page name collapsible `preparePdfPageNameSection` |
...
| Macros / Keyboard Shortcuts | `macrosModal` or `statusBarMacros` — modal listing M/S/C/L/J/P/D/H/N/R/Esc/arrows/Enter/Ctrl+Z/Ctrl+Shift+Z/Ctrl+R shortcuts |
...
- **Hotkeys** — M (Move), S (Set Scale), C (Counter), L (Quick Line), J (Snap to horizontal/vertical), P (Polyline), D (Measure), H (Highlight), N (Note), R (Rotate page); Ctrl+R / Cmd+R (Refresh); ignored when focus is in input/textarea/contenteditable (Escape still closes modals)
...
- **Turn In** — Button labeled "Turn In Project"; always does canvas-only save (performAutoSave when dirty) + check_in_project; PDF upload skipped; toast when PDF still local ("PDF saved locally—use Save Project to add it to the project"); `withTimeout` on performAutoSave and check_in_project RPC to prevent hang
