...
|------|------:|------------------|
<<<<<<< HEAD
| [app.js](app.js) | 6,671 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
...
=======
| [app.js](app.js) | 6,643 | **The remaining monolith** — down from 16.2k (9.9k after save-engine Stage 6, 8.1k after the Tier-2 splits, then −987 from the canvas-draw extraction). The only file worth actively shrinking; the region table below says what's left and in what order. |
...
| [annotation-model.js](annotation-model.js) | 619 | Done — extracted canvas/annotation data model + node tests. |
...
| `features/*.js` (55 files) | 12,545 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
=======
...
| [annotation-model.js](annotation-model.js) | 643 | Done — extracted canvas/annotation data model + node tests. |
| [undo-stack.js](undo-stack.js) | 152 | Done (2026-07-30) — `createUndoStack(ctx)` split out of annotation-model.js: the model is pure-ish data transformation, the stack is a command-history controller with UI side-effect hooks in its ctx. Covered by the undo tests in [annotation-model.test.js](annotation-model.test.js) (interleaved with model tests, dual-require). |
| [icons.js](icons.js) | 531 | Bundled icon data, mostly literals. Leave. |
| [report.js](report.js) | 496 | Self-contained report builder with a frozen `window.*` contract. Leave. |
| `features/*.js` (55 files) | 12,567 total | Healthy: largest after load-project are quick-modals (462), user-activity (459), user-admin (453), room-sizer (443), output (416), scale (412) — each single-feature scoped with its own Playwright spec. Leave. |
>>>>>>> claude/dup-counter-ids
...
- L176 - State
- L339 - [sync] Sync recovery & client recycle
- L420 - [sync] Global force reload
- L504 - [sync] Save Status log & envelope
- L507 - [sync] Field-error telemetry
- L566 - [sync] Dirty tracking & local session reset
- L572 - Undo/redo stacks
- L703 - [sync] Checkout probe, hashing & PDF cache
- L765 - Math & Format Helpers
- L1178 - Coordinate Helpers
- L1186 - PDF render bitmap cache
- L1240 - Sharp crop tile (deep-zoom sharpening + window-first commits)
- L1251 - PDF Rendering
- L1958 - UI Render Functions
- L2519 - Inline rename & polyline edit mode
- L2633 - Modal primitives (showModal / hideModal)
- L2663 - Toasts & line color picker
- L2717 - Airboard cloud sync
- L2762 - Supabase RPC & presence heartbeat
- L2802 - User activity / event telemetry
- L2845 - Supabase auth & dev auth
- L2977 - [sync] Checkout subscription & permission refresh
- L2987 - Modals & Handlers
- L3055 - PDF intake (upload, test PDF, hashing)
- L3063 - Toolbar tool buttons
- L3176 - Tool sidebar buttons & legend overlay
- L3266 - Add Line Type modal
- L3351 - Line color & sidebar handlers
- L3529 - Polyline modal & drawing
- L3560 - Zoom bar & page navigation
- L3586 - Export canvas JSON
- L3602 - PDF download helpers
- L3611 - View-link URL helpers & show-highlights/notes
- L3683 - Custom icon upload handler
- L3693 - Export & report dropdown menus
- L3780 - Sidebar drawer toggles
- L3791 - Mobile actions burger menu pointer & header logo
- L3803 - User Activity pointer (format.js + features/user-activity.js)
- L3815 - My Settings pointer (features/my-settings.js)
- L3838 - Auth & settings entry buttons
  - L3883 - Project Settings checkout & Save Status bell
  - L3984 - [sync] Checkout expired recovery
  - L4040 - [sync] Turn In
  - L4142 - Share modal pointer & copy-project openers
  - L4173 - Settings menu actions
  - L4194 - Auth sign-in form
  - L4218 - Save Project modal
  - L4231 - Checkout expired recovery modal wiring
  - L4336 - Last-session restore prompt
  - L4343 - Canvas Repair modal wiring
- L4496 - Canvas Event Handlers
- L4890 - Event Binding
- L4900 - Aim loupe (mobile press-hold precise placement)
- L5039 - Zoom transform preview & commit
- L5118 - Canvas mouse, wheel & touch handlers
- L5779 - Global dropdown dismissal & keyboard hotkeys
- L6041 - [sync] Manual save to cloud
- L6051 - [sync] Auto-save
- L6058 - [sync] Local backup (IndexedDB takeoff state)
- L6191 - [sync] Checkout keep-alive
- L6205 - App feature registry
- L6466 - View-only mode
- L6472 - Init / boot
