...
- **Sheet-size correction for compressed PDFs** — Detects when a PDF was rescaled from its original sheet size and corrects preset scales, so a half-size print doesn't silently cut every length in half.
...
- **Artboard (cloud palette)** — Your counters, line types, modifiers, and Quick Key layout follow your account to any device — set up once, reuse every bid.
