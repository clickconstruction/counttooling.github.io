...
- **Fittings from bends** — Turn it on for a line type and every run of that type counts its own elbows: each bend nearer 45° or nearer 90° is that fitting, each drop at an end is a 90, and the fitting each class produces is yours to name and count ("1/4 bend" for DWV, "45° elbow × 2" for a swept 90). Derived rows under the type in the Summary and every export, never marks; a chip at each bend shows what the tally will say; a Bid Check row beside the hangers row. When the read is wrong, edit the run and right-click the vertex: "No fitting here" for a jog drawn to route around text, "Count as 45" or "Count as 90" to force one, "Read from the angle" to take the override back.

## Measuring

- **Quick lines & polylines** — Trace straight runs and bending runs alike and get the real-world length off the page scale instantly. With a line type active, P starts tracing immediately — no dialog.
...
- **Two sample plans**: the **design-build sample plan** (an office TI, A-101, with a room schedule, panel and equipment notes and no MEP drawn — the sheet the three five-minute tours walk and the design lessons will) and the **engineered sample plan** (a restaurant plumbing sheet with its cook line, dish pit, water heater and CW / HW / gas piping drawn — the sheet for learning to read what an engineer drew), both one click from the empty canvas or Project Settings and opened through the ordinary intake.
- **Plan-and-spec callouts**: On an engineered M-sheet the trace reads the printed duct sizes out of the PDF's text layer: arming Duct beside a callout pre-fills the starting size ("from the plan: 24×12 near the cursor" — or, with nothing near the cursor, the one size the sheet prints: "… — the plan's only printed size"), and passing a different size mid-trace offers it as a quiet chip ("Plan says 20×12 here — S accepts") and as the first choice in the `S` popover — never applied on its own; scans simply behave as before.
...
- **Open in TakeoffTooling** — Hands an electrical takeoff to TakeoffTooling (assemblies, MC labor units, supplier pricing) as a structured payload: every row carries its unit (ea / ft / px), its group, its pages and its child counts nested under it, plus the project name and the plans link — nothing is retyped or re-inferred on arrival. Runs behind the same pre-copy scale check as Copy to PipeTooling.
...
- **Check-out / turn-in (one editor at a time)** — Two people can never save over each other; the lock frees itself after 30 idle minutes so a project is never stuck. Turning in offers Close project right on the confirmation, a [Close] appears in the header beside the edit-status banner while you view a project you edited this session, and the same door sits at the foot of the header cloud menu — no more reloading the tab to get out.
