...
> **Re-stamped 2026-09-21.** All fifteen planned articles are published (26 → 27 with
> `reviewing-all-bids` and `duct-takeoff-by-the-pound`), the index grew learning paths
> (`/guides/path/<track>/`) instead of category headings, and the J-snap drift is fixed.
> The drift guard became a test, not a convention: `teaching-labels.test.js`. What keeps
> the guides true from here is [journeys/plans/LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md).
