...
> readout, room type not on the Room Size dialog). **J5 / J6 / J11 re-walked
> the same day** (drift-patrol sections at the foot of each dossier): the three
> plumber/electrician routes hold post-duct with zero new chrome on the plan,
> tallies, legend rows, context menus and copy pastes — the 2026-08-30 batch is
> what changed their steps, not the build. Nine new findings, all re-driven:
> **3 stumbles awaiting re-rank** alongside J19's six — (J5-A) S mid-draw drops
> the tool but not the draft (pre-existing `setScaleClick` reset; the seam the
> duct trace guards and the line tools don't), (J5-B) a polyline draft and a
> duct draft can be alive at once — both finish bars stack and Esc unwinds the
> wrong one, orphaning the duct bar, (J6-G) **multiply zones skip duct** — a ×3
> typical floor multiplies the diffusers and leaves the run at ×1 with the
> dialog previewing "0 line run(s)" (⚑ blocker candidate by the J6 #10
> precedent; `gap`, product call) — and six papercuts folded into **B19**
> (HVAC fields on every trade's counter dialogs, Duct inline while Polyline is
> behind ⋯ — ⚑ contra D14, duct toggles in Legend Settings, Delete Area ignores
> duct, Copy Summary pastes the duct panel's setup hints). The **day-7
> `duct_run` telemetry line is due ~2026-09-19** (migration applied 2026-09-12).
...
| 26 | ☑ merged 2026-08-30 | Trimming a 200-sheet set is strictly one-sheet-at-a-time — ~185 clicks to reach a 15-sheet P-set; fatigue defeats the feature (J2) | features/prepare-pdf.js (only Prev/Next/Delete/Undo/Rotate exist) | Thumbnail-grid trim with tap-to-keep/drop, single-sheet preview for zoom — must REPLACE the sheet walk as default, not sit beside it (largest build in this set) | L | new `prepare_trim` event |
| 27 | ☑ merged 2026-08-30 | Double-click sheet rename is dead — row onclick → fitZoom → renderPagesList innerHTML rebuild destroys the node before the second click; the working badge-click path is advertised only by a hover tooltip (J2) | sidebar renderPagesList rebuild race | Skip the rebuild when the clicked row is already active / defer past the dblclick window — repairs the affordance with zero new UI | S | none |
...
| B19 | Duct drift-patrol batch (J19, 2026-09-13 — the confirmed papercuts; the six stumbles wait for the Tier-2 re-rank) | Give Duct a hotkey (an unbound letter, e.g. U) in HOTKEYS so the ⋯ row and Keyboard Map stop teaching a blank (hotkeys.js, header-more.js `key: ''`); Bid Check manual-row LABEL click ticks the row like the box (features/bid-check.js:178 — Will's chip task_4c782a3d); Copy Schedule shows ONE toast — fold the open-items count into "Duct schedule copied — Bid weight …" or skip the post-action advisory on the duct copy (features/duct-schedule.js + bid-check.js showBidCheckAdvisory); the export gate on an AUTO ⚠ row gets an acknowledge so a deliberate under-serve stops re-asking on every press (features/duct-bidcheck.js gate — ⚑ borderline on spirit (3), Will's call); Create tab: a typed CFM (or an HVAC project trade) switches to the Custom Icons panel and hoists the HVAC group first like the Quick tab (icon-render.js `firstSet`), and icon cells get a name tooltip; flex-drop sublabel + placeholder "empty = 8'" → 5' to match DUCT_FLEX_DEFAULTS.dropFt (app/index.html:692-693); Seam & waste % input gets the `duct-schedule-num` class (400 px wide today, features/duct-schedule.js); draw a small marker + "3' ↑" tag at rise/drop vertices (canvas-draw.js never reads `verticalFt`); **known build item:** `build:icons` has no `--check`, so a stale icons-custom.js is not caught by `npm run check` (scripts/build-custom-icons.js, scripts/check.js STEPS). **Re-walk additions (J5 / J6 / J11, 2026-09-13):** trade-gate the counter dialogs' device fields — CFM + Flex drop only for HVAC (or once a CFM is typed), Mount height only for Electrical, or one closed "Device details" disclosure (app/index.html `#counterCfm` / `#counterFlexDrop` / `#counterQuickCountCfmRow`; J5-C); Duct joins the ⋯ overflow group like Polyline — or the strip goes trade-aware (features/header-more.js `strip: true`, styles.css:241 hide list; ⚑ contra D14's "keep the strip order" — Will's call; J5-D); Legend Settings hides "Show duct rows" / "Show duct true width" (and "Show room volumes") until the project has them (features/legend-settings.js; J5-E); Delete Area counts and removes duct runs + their fittings and previews "N duct run(s)" (app.js `deleteZonePreview` + the delete walk; J6-H); the email summary's Bid Check block skips `na` rows (or prints label-only) so "Give a room a type on its Edit Room dialog…" stops riding the bid email — pair with J19 #4's duct block (report.js email block, features/duct-bidcheck.js `na` details; J11-I). Stumbles from the same re-walks (J5-A S-mid-draw limbo, J5-B two live drafts, J6-G multiply zones skip duct) wait for the Tier-2 re-rank with J19's six · **J19 stumbles 4–6 (re-ranked into this batch, Will 2026-09-13):** device attachment feedback at the 8" tolerance ("not attached — move closer or tap to attach"); footer keeps the "S = size" hint + live readout at ~1380 px with a save stamp (compact the stamp first); room type on the Room Size dialog · **Ratchet 2026-09-13 ("is this the best we can do?", each row):** hotkey via the HOTKEYS single source (D if free) incl. the button tooltip; icon landing = an inline "→ Supply Diffuser · change" chip beside the CFM field, no tab switch; gate = "Export anyway" IS the acknowledgment (remember the unresolved set, re-fire only when it changes — no Acknowledge control); rise/drop entries join the fitting-marker family (same menu/hit-test/reconcile); air &amp; mounting fields fold under a "More ▸" disclosure (auto-open on HVAC/Electrical profiles, remembered per project) — hiding them would make the first CFM device impossible; attachment made VISIBLE by dashed flex leaders device→run (the original mockup had them) + tap-the-run-to-attach; flex label reads the data-table default so it can't drift · **Shipped 2026-09-13:** part 1 as D18, part 2 as D19 (duct-b19a / duct-b19b specs); the X-ladder that followed (D20–D25) closed X1–X4, X6 and J5-A/J5-D — see _INDEX-DUCT.md Wave 3 |
