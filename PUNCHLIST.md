...
| FILM-HOMERUN | Electrical film: the home run to LP-1 is drawn with Quick Line but never flagged as the homerun, so Bid Check's voltage-drop row likely reads "needs a panel mark or a homerun" on camera (found building SPOT-2, where the flag made it compute); verify on the film, then flag it on camera (Line Properties → Homerun) and re-render | bug | agent | — | [LANDING-REFRESH.md](journeys/plans/LANDING-REFRESH.md#the-per-trade-films-2026-09-17-in-progress) |
| HERO-CHAPTERS | The hero films get a chapter strip over the frame: Scale → Count → Pipe → Bid as story segments, a question with a real-time clock answering it, the film holding on its end screen with Play again. Decided on a mock, not built; the strip still wants a cleaner look and the question's wording is open | build | dev | — | [LANDING-REFRESH.md](journeys/plans/LANDING-REFRESH.md#the-hero-chapters-plan-of-record-2026-09-19) |
| SPEC-TURNIN | turn-in-self-release.spec.js fails on main: its Prepare PDF step waits for `#preparePdfProjectName`, an id that does not exist in the shell (the field is `#preparePdfName`); cloud-gated, so CI never sees it, but a local run with dev auth always reports it red | test | agent | — | [turn-in-self-release.spec.js](turn-in-self-release.spec.js) |
| CONFIRM-ROUTE | Delete zone and Delete room are still their own dialogs (`#deleteZoneModal`, `#roomDeleteConfirmModal`); fold them into `App.confirmDialog` so the app has one confirm (copy and buttons already match the template) | chore | agent | — | [CHANGELOG.md](CHANGELOG.md#stylemodals-the-reviews-last-fifteen-asks-2026-09-19) |
