...
| DEV-AUTH | The dev-auth test credentials in the main checkout's `config.local.js` are rejected by Supabase ("Invalid login credentials", 2026-09-19), so every cloud-gated spec self-skips on this machine; reset the test account's password (or update the file), then run turn-in-self-release, indexeddb-backup and the load-project specs green to confirm SPEC-TURNIN and SPEC-DUPES | chore | dev | — | [CHANGELOG.md](CHANGELOG.md#chorepunchlist-the-five-agent-rows-worked-in-one-sitting-2026-09-19) |
...
| SPOTLIGHT-SYNC | Landing spotlight: the twelve electrical and HVAC frames are cut from their own small seeded takeoffs, so six captions quote numbers the films no longer show (HVAC "174 lb" and "600 of 2,000 CFM" against the film's 1,422 lb and 2,400 of 3,000; electrical "6 rows" and "2.3%"). Point `electricalBase` and the HVAC seed in scripts/build-screenshots.js at the films' layouts (three circuits; the full floor off RTU-1), re-cut the twelve frames with `--set spotlight`, update the six captions in index.html. No film render needed | chore | agent | — | [LANDING-REFRESH.md](journeys/plans/LANDING-REFRESH.md#the-trade-spotlight-plan-of-record-2026-09-18) |
