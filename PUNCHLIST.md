...
| RESTORE-LATE | The "Project from Last Session" prompt can open over work already started. It is offered when boot's sign-in resolves and, if a dialog is up, deferred and retried every second (features/restore-last-session.js), with no check that the user has since opened something: on a slow connection a user uploads a plan, answers Load Annotations, and the restore prompt lands on top of their Save dialog (seen with every Supabase call delayed 2.5 s). Decide the rule (drop the offer once `state.pages.length > 0`, keeping the backup for next boot the way the dismiss helper does) and mind the T1-01 clobber guard, which suspends takeoff backups while a prompt is pending | bug | dev | — | [CHANGELOG.md](CHANGELOG.md#testturn-in-the-flag-on-turn-in-wait-reports-why-it-timed-out-2026-09-20) |
