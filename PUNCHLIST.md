...
| R2 | Apply the `log_user_event` allowlist catch-up: 15 client event types 400 today; migration `20260916143700` + drift test are written, applying needs the `pg_get_functiondef` diff first | build | dev | ⚑ Will's go | [_TODO.md](journeys/plans/_TODO.md#r2--13-client-event-types-400-against-the-deployed-log_user_event-allowlist) |
| D26 | Drift patrol after the sample-plan promotion: re-walk J5, dated addenda on a dozen dossiers | chore | agent | — | [_TODO.md](journeys/plans/_TODO.md#d26--drift-patrol-after-the-sample-plan-promotion) |
...
| J6-G | Multiply zones skip duct: a ×3 typical floor multiplies the diffusers and leaves the run at ×1, dialog previews "0 line run(s)" | bug | ⚑ call | — | [_STAGE6.md](journeys/plans/_STAGE6.md#re-rank-inputs-from-the-drift-patrol) |
| J5-B | A polyline draft and a duct draft can be alive at once: both finish bars stack, Esc unwinds the wrong one | bug | dev | — | [_STAGE6.md](journeys/plans/_STAGE6.md#re-rank-inputs-from-the-drift-patrol) |
| J19-RERANK | Re-rank J19's six stumbles (hidden groups toggle, deck height before the first run, silent 8" device attachment, duct missing from Copy Summary and Copy to /Tooling, laptop-width footer, room type off the Room Size dialog) | decision | ⚑ call | — | [duct-takeoff.md](journeys/duct-takeoff.md#friction-findings) |
| X6-SLOT | Bookkeeping: `_STAGE6.md`'s inline X6 `[decision] ____` is still empty although X6 was decided 2026-09-13 and shipped as D25 | chore | agent | — | [_STAGE6.md](journeys/plans/_STAGE6.md#the-two-product-calls) |
