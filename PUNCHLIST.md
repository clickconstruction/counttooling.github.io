...
| LEARN-GUIDES | The plumbing, electrical and HVAC guides rewritten to follow their tours, every claim walked in the app, with screenshots | build | agent | — | [LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md#phases) |
| LEARN-ENGINE | Tour engine hardening: a step closes the dialogs the last one left open, the restore offer never lands on a `?tour=` start, a tour never inherits a device backup's marks, touch wording | build | agent | — | [LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md#phases) |
| LEARN-LESSONS | The Learn menu and its thirteen short lessons on the engineered sample plan, dormant behind `?ff=learn` | build | agent | LEARN-ENGINE | [LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md#the-lessons) |
| LEARN-FLIP | Learn on for everyone: drop the flag, "Try it" links from the guides, `tour_step` names the lesson | build | agent | LEARN-LESSONS | [LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md#phases) |
