...
| VIEWER-HIDEMARKS | A view-link recipient's Hide marks eye has never been checked across the matrix the 2026-08-31 cloud walk flagged: the desktop header, the phone ☰ row, and the read-only viewer where the mark tools are gone. view-only.spec.js and hide-marks.spec.js both run as the owner, never as a viewer. Walk it, or pin it in view-only.spec.js | test | agent | — | [_NEXT.md](journeys/plans/_NEXT.md#stage-5---supervised-cloud-interior-walk-schedule-with-will) |
