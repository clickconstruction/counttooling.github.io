...
| COURSE-WORDING | Step cards in the plumbing tour and the HVAC and electrical courses that describe the screen wrongly (a card "above" that is below, a pencil that is not there, field labels that differ, a copy button with nothing to copy) or give no hint for a common miss | bug | agent | — | [LEARN-PLAN.md](journeys/plans/LEARN-PLAN.md#card-wording-that-does-not-match-the-screen-the-2026-09-24-read) |
| LEARN-LEAK | Counters and line types a reader makes by hand inside a lesson or course (the Create tab, the Quick tab, + Add) are never flagged as the lesson's, so leaving the lesson keeps them in the palette and they ride into the next real bid (found by hand 2026-09-25: "HB Hose Bibb", "1.5in Copper" and the rest were still there a chapter later). Decide: sweep everything made while a lesson set is open, or keep what the reader made | decision | ⚑ call | — | — |
| PERSONA-BUILD | The persona seams: the lessons' step manifest and label index, reason codes on hints, `App.tutorialObserve()`, the one-browser harness and the merge script, named devices | build | agent | — | [PERSONA-PLAN.md](journeys/plans/PERSONA-PLAN.md#build-items-in-order) |
| PERSONA-CALIBRATE | Run the personas on the plumbing tour at the commit before #199 and score them against the six by-hand walks' findings (recall and noise) before any persona output is trusted | test | agent | PERSONA-BUILD | [PERSONA-PLAN.md](journeys/plans/PERSONA-PLAN.md#runs) |
| PERSONA-RULES | Steps that teach a code value name their rulebook ids, and `npm run check` fails a card whose number or section has no rule or disagrees with it | build | agent | — | [PERSONA-PLAN.md](journeys/plans/PERSONA-PLAN.md#build-items-in-order) |
| PERSONA-PASS | The text pass over every tour, lesson and course, then the live pass on the steps it flags, triaged into fixes, drafts, tester rows and calls | test | agent | PERSONA-CALIBRATE | [PERSONA-PLAN.md](journeys/plans/PERSONA-PLAN.md#who-does-what-the-owner-gives-as-little-input-as-possible) |
