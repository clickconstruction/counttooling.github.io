...
**log_user_event_allowlist_catchup** (`20260916143700`) — R2: re-creates `log_user_event(text, uuid, jsonb)` from the latest body in the chain (`20260906120000_log_user_event_duct_run`) with the fifteen event types the client already sends but every deployed body rejected with "invalid event type": `project_close`, `restore_prompt_deferred`, `tour_step`, `trade_set`, `codes_set`, `ceiling_set`, `drop_set`, `bid_check_row_state`, `child_count_from_rule`, `rule_open`, `tag_suggestion_accepted`, `ghost_placed`, `ghost_stamped`, `client_error`, `client_unhandled_rejection`. Before applying, diff its list against `pg_get_functiondef` on prod (the file says how). [log-user-event-allowlist.test.js](log-user-event-allowlist.test.js) (Node, CI) keeps the newest allowlist migration a superset of every client `logUserEvent('…')` literal and of every earlier migration, so the list cannot drift again.
