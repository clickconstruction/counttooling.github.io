...
- **profiles** — `user_id`, `is_admin` (identifies admins; later migrations add `is_digital_twin` and `is_overseer` — see **profiles_is_overseer**)
...
**log_user_event_allowlist_catchup** (`20260916143700`) — R2: re-creates `log_user_event(text, uuid, jsonb)` from the latest body in the chain (`20260913025935_log_user_event_duct_run`) with the fifteen event types the client already sends but every deployed body rejected with "invalid event type": `project_close`, `restore_prompt_deferred`, `tour_step`, `trade_set`, `codes_set`, `ceiling_set`, `drop_set`, `bid_check_row_state`, `child_count_from_rule`, `rule_open`, `tag_suggestion_accepted`, `ghost_placed`, `ghost_stamped`, `client_error`, `client_unhandled_rejection`. Applied 2026-09-16 via `supabase db push --linked` after `supabase migration fetch` confirmed the deployed body (the duct_run re-creation, recorded on prod as `20260913025935`, which is why that file carries that stamp in the repo). [log-user-event-allowlist.test.js](log-user-event-allowlist.test.js) (Node, CI) keeps the newest allowlist migration a superset of every client `logUserEvent('…')` literal and of every earlier migration, so the list cannot drift again.
...
**profiles_is_overseer** — Adds `profiles.is_overseer` (read-only oversight role, e.g. Master Malachi) plus two additive read-only policies: "Overseers can view all projects" (projects SELECT) and "Overseers can read all PDFs" (storage SELECT, mirror of the admin policy). Deliberately **not** added to `user_can_access_project()` — that helper also gates `project_shares` INSERT and the view-link policies, and an overseer must stay read-only. The `check_out_project` guard stays owner/editor/admin, so an overseer can never acquire edit rights.

**list_accessible_projects_overseer** — Extends `list_accessible_projects` with the overseer arm in its WHERE clause; `my_access_role` reports `'viewer'` for overseers (so all existing viewer-mode client behavior applies) and `can_check_out` is unchanged — an overseer's rows always open read-only. Powers the **Bid Board** (`features/bid-board.js`) and the overseer's Load Project view (incl. the owner-email filter, previously admin-only).

**overseer_admin** — Adds admin-guarded RPC `admin_set_overseer(uuid, boolean)` (upserts the profile flag; the Manage Users eye-toggle's backend) and drops/recreates `list_users_for_admin()` with `is_overseer` + a three-way `role` label (`Admin` > `Overseer` > `User`). The `admin-list-users` Edge Function mirrors the same mapping (redeploy it).

**project_review_flow** — The bid review handoff (estimator → overseer): adds `projects.review_status` (`null` | `ready` | `reviewed`, checked) with `review_requested_at/by` + `reviewed_at/by` audit columns, and the single transition RPC `set_project_review_status(uuid, text)` — `'ready'`/clear require owner, editor share, or admin; `'reviewed'` requires overseer or admin. Setting `ready` clears the reviewed pair; clearing nulls everything.
...
| `PT_VIEW_GRANT_SECRET` | `get-view-project` | HMAC secret behind **viewer grants** (2026-09-06): PipeTooling's sub-portal function signs a short-lived, token-bound grant (`&g=` on a view link) that lets a subcontractor open plans from their portal without the email gate. The same value lives in PipeTooling as `COUNTTOOLING_VIEW_GRANT_SECRET`. Rotating it makes every outstanding grant fall back to the email gate — nothing breaks, subs just get the prompt until their portal re-mints. Unset = grants are refused (`no_secret`), the gate applies to everyone. |
...
**View links:** `get-view-project` validates tokens and email domain (default: clickplumbing.com). **Viewer grants (2026-09-06)**: a request carrying `grant` (from `&g=` on the link) is verified with `_shared/viewGrant.mjs` against `PT_VIEW_GRANT_SECRET` — signature, `exp` (≤ 24 h), `iat`, a known `via`, and `claims.t === token` — and, when valid, skips the domain gate and logs the visit with `viewer_name` + `source` (migration `20260906000000`); an invalid grant answers `403 grant_invalid` and the client falls back to the email gate. The kernel is plain ESM + Web Crypto so Node's runner covers the file Deno runs (`view-grant.test.js`). Set `VIEW_LINK_ALLOWED_DOMAINS` in Supabase Dashboard (Functions > get-view-project > Secrets) if different. Dev/bot accounts on douglasmining.com are a standing exception: `_shared/viewLink.ts` keeps a separate exempt list (`VIEW_LINK_EXEMPT_DOMAINS` secret, default douglasmining.com) that always passes the gate but is never shown in user-facing copy (the rejection message, the email-gate placeholder, and the Share modal all render only the allowed list). The function also returns the project's `updated_at` (as `updatedAt`); the client (`initViewOnlyMode`) revalidates against the server on open and only re-renders/refreshes the view cache when it changed, so a viewer isn't pinned to a stale snapshot after the owner re-saves (the cached PDF blob is reused when its hash matches; offline falls back to the cache). The deployed function inlines its CORS headers (the repo source imports `_shared/cors.ts` — functionally identical; a CLI deploy bundles `_shared`). `set-view-scale` (same token + domain gate, also reads `VIEW_LINK_ALLOWED_DOMAINS`) lets a viewer set a page's scale for everyone: it sanitizes the scale payload and writes `projects.data.pages[i].scale` with a `viewerSet {email, at}` stamp that drives the owner's must-clear notice in the app.
...
- **Overseer / Bid Board** — A user flagged `profiles.is_overseer` (Manage Users 👁 toggle, or SQL upsert like the admin one above) sees **every project, always read-only**: the **All Bids** board (sidebar button; auto-opens for pure overseers signing in with nothing to restore) lists every bid as cards (name, estimator, counts, last edited) with search + estimator filter, and clicking one opens it in the existing viewer mode. Load Project shows the same full list with the owner filter. No edit, checkout, share, delete, or admin surfaces — enforced server-side (dedicated read-only RLS policies; the checkout RPC refuses overseers). Test-harness accounts' projects are hidden from the board.
- **Bid review handoff** — An estimator marks a cloud bid **Ready for review** from Project Settings (status row + toggle; withdraw or re-request anytime). The Bid Board pins ready bids in a **Ready for review** lane where an overseer/admin clicks **Mark reviewed**; reviewed bids carry a quiet ✓ badge (Load Project rows badge both states too). All transitions go through `set_project_review_status` (server-guarded per role).
