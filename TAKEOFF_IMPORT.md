...
| palette `tag` (counters) | the fixture tag the plan writes beside the symbol — `A`, `B1`, `EM` | `tag` on the counter: tag-aware placement lands a click on the counter whose tag matches the text-layer token beside it (a name like "Type B" reads the same way) |
| `bidCheck` | `{ manual?: { <row-id>: true }, loadAmps?, volts? }` — the Bid Check's manual ticks (ids from `BID_CHECK_MANUAL_ROWS`) and the circuit load / voltage the voltage-drop row assumes (app defaults 12 A · 120 V) | `state.bidCheck`; the auto rows are computed, never sent |
