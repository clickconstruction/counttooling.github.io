...
**New here?** Open the app with nothing loaded and click **electrical** under *take the five-minute tour* (or open it directly: [the electrical tour](/app/?tour=electrical)). It walks this article on the sample plan (set and prove the scale, switch the trade, add a duplex receptacle, count three, make a 3/4" EMT type with its conductors, set the ceiling, chain a run with its verticals, make it circuit LP-1 / 7, read the wire, open Bid Check, hand it off), checking each step against what you actually did, with a *Do it for me* button on every step. The picture below is the takeoff that tour ends with.
