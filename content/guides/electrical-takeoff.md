...
Make a [counter](/guides/counting-with-counters/) [[counter]] per device type — receptacles, switches, fixtures by type, data drops, panels — each with its own color and icon so the sheet stays readable ([upload your own SVG symbols](/guides/custom-icons/) to match the fixture schedule). Then click through the sheet; every click is one tally, rolled up across the whole set.
...
Trace runs with **Line** [[line]] and **Polyline** [[polyline]] against the page scale. Three details matter for electrical:

- **[Drops](/guides/measuring-runs-lines-and-polylines/)** add vertical footage where a run turns up a wall or rises between floors — the footage plan-view takeoffs systematically miss.
