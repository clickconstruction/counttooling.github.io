...
**New here?** Open the app with nothing loaded and click **hvac** under *take the five-minute tour*. It walks a design-build duct takeoff on the sample plan (set and prove the scale, box a room the plan already names, give diffusers a CFM, name the system, let the app size the main at `S`, hang the strays, read the pounds, sign off and hand it to the bid). It circles where to click and shades where to drag on the sheet, checks each step against what you actually did, and *Show me where* points at anything you cannot find. Open it directly: [the HVAC tour](/app/?tour=hvac). For the why as well as the how, [the HVAC course](/app/?course=hvac) is nine chapters on the same restaurant's mechanical set: which room breathes hardest, why the main shrinks down the hall, whether the wrapped duct fits the plenum and the fan can push to the far diffuser, and the duct that must never be galvanized ([Learn CountTooling by doing](/guides/learning-the-app/) lists them).
