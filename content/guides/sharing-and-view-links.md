...
- **Every mark identifies itself.** Tapping a counter marker peeks its name and number (*"Water Closet, #4 · 7 on this page"*), and tapping any drop marker peeks its size in a chip (*"3 ft drop"*). The **Drop sizes** [[drop-sizes]] toggle labels every drop on the sheet at once, off by default so the drawing stays clean, and remembered per link like Hide marks. Risers and stacks answer themselves instead of prompting a call. (Full detail: [reading drops back](/guides/measuring-runs-lines-and-polylines/).)
