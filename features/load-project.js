(function () {
  ...
  function buildLoadProjectRowHtml(proj, sizeBytes) {
    ...
    // Bid stamp leads the meta line — it's the strongest identifier, not a status.
    const refChip = proj.external_ref ? '<span class="bid-stamp" title="Upstream bid ' + esc(proj.external_ref) + '">' + esc(proj.external_ref) + '</span> ' : '';
    const metaParts = [date, sizeStr].filter(Boolean);
    const meta = refChip + esc(metaParts.join(' · ')) + lockBadge + ownerBadge + reviewBadge;
      ...
      if (proj.pdf_path) {
        try {
          ...
          if (!buf && deviceBuf) {
              ...
              if (d.waterSettings) state.waterSettings = { ...App.state.waterSettings, ...d.waterSettings };
        ...
        if (canvasData.waterSettings) state.waterSettings = { ...state.waterSettings, ...canvasData.waterSettings };
