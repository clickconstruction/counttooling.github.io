...
(function () {
  ...
  function render() {
    ...
    else if (step.reveal && !revealed) { actionBtn.style.display = ''; actionBtn.textContent = step.revealLabel || 'Show the engineer\'s answer'; }
  ...
  function startTutorial(id) {
    ...
    active = true;
    stepIdx = 0; doneAt = 0; heldByBack = false; revealed = false; dragPos = null; tourCounterId = null; tourLineTypeId = null; tourSecondCounterId = null;
  ...
  el('tourAction') && (el('tourAction').onclick = async () => { const step = STEPS[stepIdx]; if (step.action) { await step.action.run(); render(); } else if (step.reveal) { revealed = true; render(); } });
  Object.keys(TOURS).filter((id) => TOURS[id].linkId).forEach((id) => {
    const link = el(TOURS[id].linkId);
