...
- **Still claimed by NO journey after J19** (walked-adjacent, not exercised — queued for the J5 / J6 / J11 re-walks): Plenum return toggle behavior; VD per tap remove/add; Boot / Offset reclassify; On edge applied to a run; multi-sheet schedule scope (This sheet / Every sheet); 2" / 3" / 1/2" pressure classes; liner sq ft; Legend "Show duct true width" off; Copy Schedule's scale gate; duct runs inside scale zones / multiply zones (J6 re-walk); duct rows in the Summary drill-down (J18); duct + Copy to /Tooling handoff content (J11 re-walk — J19 found the copy carries no pounds).
