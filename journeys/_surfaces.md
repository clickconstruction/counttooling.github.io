...
- **Canvas right-click:** fitting marker or run stretch → #ductFittingMenu (non-modal; heading "90° elbow · 22×16 · auto" / "Run · 24×12 → 22×16"; 45° elbow · Transition · Tap · Boot · Offset · Delete fitting · Remove/Add volume damper · Orientation Flat | On edge · Delete run); CFM-carrying marker → #contextMenu #ctxMarkerCfm "CFM for this one…" → `markerCfmModal`. WSFU-carrying marker → #ctxMarkerWsfu "WSFU for this one…" → `markerWsfuModal` (WATER-PLAN rung 2, features/water-fixtures.js). A fixture with a side no water run serves and a run of that side in reach → #ctxAttachWater "Attach to nearest cold run" / "… hot run" / "… cold and hot runs" (rung 3; stores marker.waterRuns, the mark stays put). **Rung 4:** a polyline traced on a water type → `#waterHintCard` above the footer ("3/4 in suggested · 3 WSFU still to serve · 4.3 ft/s · S accepts") and S → `#waterSizePopover` (non-modal, the duct popover's markup: Suggested, then every size of the material with its velocity; a tap ends the run at its last point and starts the next there in the sized type).
