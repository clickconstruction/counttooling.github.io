...
| 7 | `exhaust` | which duct must not be galvanized (trace it with Material set to welded black steel; galvanized is sent to the run menu); why the gauge table does not apply and what the Schedule did (reveal); where a duct crosses the rated wall (count a Fire Damper at both; a wall that is not rated is refused with the keynote); why the grease duct would still get none (reveal: NFPA 96); why make-up air (reveal) | the run's material and its own schedule row at 16 ga, the Fire Damper quick type, the restroom exhaust as an exhaust run at 8"ø, the make-up duct at 20×16 and MA-1 counted |
