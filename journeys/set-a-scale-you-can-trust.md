...
Personas: P E H · Status: ● walked 2026-08-02 (desktop 1380×900 + mobile 375×812, headless Chromium, samples/sample-plan.pdf) · re-walked 2026-09-16 (D26, promoted sample plan: the sheet warning and findings #1–#3 no longer reproduce on the sample, see the addendum at the end)
...
## Re-walk 2026-09-16 (D26, after the sample-plan promotion)
...
Walked as the Phase-2 recipe: headless Chromium 1380×900 against a static server on :4577 serving main @ 2584940, `samples/sample-plan.pdf` through `#pdfInput`, signed out, zero console errors end to end. Scratch driver, not a spec; every number below was read from `state` or the DOM.

**Preset route today (2 actions, 0 decisions):**

1. S. The modal opens on "Architectural & Engineering", 22 presets, no yellow warning (`#scaleSheetWarning` hidden, `#scaleSheetSelect` empty). The card is 765 px tall at 900 px viewport (top 68, bottom 833) with both tabs on screen at y = 131; "Select two points" clicks first time. The blue advisory reads "Presets are an estimate. They assume the plan is printed to true scale. After applying, confirm by measuring two known points." with its own "Verify by measuring two points" button.
2. Click "1/8" = 1'". Applies and closes: `page.scale = { pixelsPerUnit: 9, unit: 'ft', label: '1/8" = 1\'' }`, no `correctionFactor`, no `sheetSize`. The T1-04 verify hand-off does not arm (`scaleCheckMode` false, tool back to Move), because there was no correction to check. Toast "Scale set. Verify it against a known dimension" (the 2026-09-14 copy pass: a period, no dash) in the corner strip, click-through. Sidebar badge "1/8" = 1' · 1 ft = 9.0 px". **The header Set Scale button no longer hides once a scale exists** (D20 / X3): it stays lit and reads "1/8" = 1'", and clicking it reopens the modal with the preset highlighted. The entry-point line above that says "Hidden in header once scale is set" is history.
3. Measure (D) across the printed 67'-4" overall: footer "Distance: 67'-6"", no overlay.

**Verify route today:** badge → "Verify by measuring two points" → modal hides, footer "Click first point" → both ends of the 67'-4" overall → the modal comes back with the check panel: "Checking against 1/8" = 1'. Enter the real length of the line you just measured." (placeholder "e.g. 10 or 10'6", unit ft) → type 67'4 → Check → **Expected 67'-4" / Current scale reads 67'-6" / "Within 0.3%. The scale looks correct."** in green, Cancel relabelled "Keep current scale", "Use measured" beside it. Kept.

**What changed under the findings:**

- **#1 (the silent ANSI-D correction) cannot happen on the sample any more.** The sheet is a true ANSI B, the detector stays quiet, presets apply uncorrected. The trap is still real in the wild: any print that is not a standard sheet with a standard aspect within `analyzeSheet`'s tolerance, for instance the same drawing printed at 75% (918 × 594 pt), or a re-boxed half-size set. A walker who wants it back has two doors: `scale.spec.js`'s `stubNonStandardSheet` (forces the ANSI_D guess) and `scripts/build-screenshots.js`'s "sheet-warning" shot, whose stub comment ("the synthetic sample plan is a standard sheet, so the real detector would stay silent") is true again after being false on the old sheet (see the 2026-08-02 verification note). The proposal shipped as T1-04 (a corrected apply flows straight into the verify, escapable) and is pinned there; on this sheet it simply never triggers.
- **#2 (the 1109 px card unreachable at 900 px) no longer reproduces anywhere.** Two things closed it: the warning that made the card tall is off on standard sheets, and the 2026-09-14 modal rule (`.modal-overlay` scrolls, `.modal-card` carries `margin: auto` and a `dvh` max-height, CHANGELOG "Project Settings reachable on small viewports") applies to every modal. On this walk the card fit outright.
