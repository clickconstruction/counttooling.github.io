...
| 9 | papercut | Access log renders as a raw browser `alert()`; anonymous viewer's mobile burger lists "Save status" | Crude but working; "Save status" is a cloud concept irrelevant to an anonymous GC | CONFIRMED — source: share-links.js:142 raw `alert('Access log:…')`; burger-menu.js:72 gates the "Save status" row on `SUPABASE_ENABLED && state.currentProjectId` only, and a view-link session *sets* currentProjectId, so the anonymous viewer gets the row → CLOSED (the alert half) B20 2026-09-14: the access log is the app's info dialog (`#confirmModal`, Close only) |
