 ...
 * and commit. See CUSTOM_ICONS.md. Subfolders of my-counters/ are further
 * icon SETS (my-counters/electrical/ — generated by
 * scripts/build-electrical-symbols.js); each entry carries `set`, and an
...
function parseSvg(content) {
  ...
  // Optional authoring metadata (the generated electrical set carries both):
  // <title> overrides the filename-derived display name; a <desc> of the form
  // "terms: a, b, c" seeds the search terms.
  const titleMatch = content.match(/<title>([^<]+)<\/title>/i);
  const title = titleMatch ? titleMatch[1].trim() : null;
  const descMatch = content.match(/<desc>\s*terms:\s*([^<]+)<\/desc>/i);
  const terms = descMatch ? descMatch[1].split(',').map((t) => t.trim().toLowerCase()).filter(Boolean) : null;
  return { viewBox, value, title, terms };
}
...
// Sets: the top-level SVGs are the original (plumbing) set; each immediate
// subfolder is a further set named after the folder (my-counters/electrical/
// → set 'electrical'). The icon pickers group the custom grid by set.
const setDirs = [{ set: 'plumbing', dir }].concat(
  fs.readdirSync(dir).filter((f) => fs.statSync(path.join(dir, f)).isDirectory()).sort().map((f) => ({ set: f, dir: path.join(dir, f) }))
);
const icons = [];

for (const { set, dir: setDir } of setDirs) {
  const files = fs.readdirSync(setDir).filter((f) => f.toLowerCase().endsWith('.svg')).sort();
  for (const file of files) {
    const filePath = path.join(setDir, file);
    const content = fs.readFileSync(filePath, 'utf8');
    const { viewBox, value, title, terms } = parseSvg(content);
    if (!value.trim()) {
      console.error('Skipping', file, '(no path/rect/circle/ellipse/line found)');
      continue;
    }
    const name = title || filenameToName(file);
    const entry = { value, viewBox, name, set };
    if (terms && terms.length) entry.terms = terms;
    icons.push(entry);
  }
}
