...
test.describe('Project Settings on small viewports', () => {
  ...
  test('order: This project (Trade, Codes, Occupancy, Jurisdiction, Ceiling, Use groups, Sheets, Quick keys), then the footer (Close project, Load project, Help, Advanced), then the one primary, Save', async ({ page }) => {
    ...
    // Download PDF became the Sheets row; Close project is a footer link. WATER-PLAN rung 1
    // (2026-09-23) added the Occupancy row under Codes, and with it the card overflows an
    // 800 px viewport, so the tops are read scrolled to the bottom (the sibling test's
    // idiom): the footer links then sit above the sticky Save, as they do on screen.
    await page.evaluate(() => { const o = document.getElementById('settingsModal'); o.scrollTop = o.scrollHeight; const c = o.querySelector('.modal-card'); if (c) c.scrollTop = c.scrollHeight; });
