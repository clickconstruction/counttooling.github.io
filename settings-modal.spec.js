...
test.describe('Project Settings on small viewports', () => {
  ...
  test('order: cloud actions, then This project (Trade, Codes, Jurisdiction, Ceiling, Use groups), then the footer links and Advanced', async ({ page }) => {
    await openSettings(page, 1280, 800);
    const order = await page.evaluate(() => ['settingsSaveProject', 'settingsLoadProject', 'settingsProjectSection', 'settingsTradeRow', 'settingsCodesRow', 'settingsJurisdictionRow', 'settingsCeilingRow', 'settingsUseGroupsRow', 'settingsQuickKeys', 'settingsTourHvac', 'settingsAdvancedBtn'].map((id) => document.getElementById(id).getBoundingClientRect().top));
