-- Rich per-user activity overview for the admin Manage Users modal. One round-trip:
-- returns a single jsonb object with identity/presence, all-time volume, per-event-type
-- breakdown, and the recent timeline (with resolved project names).
--
-- Admin-gated via the `guard` CTE: a non-admin caller (or a null p_user_id) sees zero rows
-- everywhere, so the function returns a fully-zeroed object -- never leaks, never errors.
-- Every aggregate scans only this user's rows (user_activity_user_id_created_idx).
...
as $$
  ...
  guard as (
    ...
    where exists (select 1 from public.profiles pr where pr.user_id = auth.uid() and pr.is_admin = true)
  ...
  agg as (
    select count(*)::bigint as total_events, min(a.created_at) as first_event_at, max(a.created_at) as last_event_at,
      ...
      -- active_days uses CST calendar dates (intentionally different from the UTC rolling windows above)
  ...
  recent as (
    ...
    from (select a.event_type, a.created_at, a.project_id, pj.name as project_name
          ...
          order by a.created_at desc limit 40) r
  )
