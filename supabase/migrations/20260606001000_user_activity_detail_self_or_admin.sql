-- Allow a user to read THEIR OWN activity detail (for the "My Activity" view in My
-- Settings), in addition to admins reading anyone's. Only the guard changes: it now
-- passes when the caller is an admin OR p_user_id is the caller's own id. A non-admin
-- asking for someone else's id still gets the fully-zeroed object (no leak).
