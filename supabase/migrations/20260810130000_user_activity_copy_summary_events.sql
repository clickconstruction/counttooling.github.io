-- T1-05 (ft/px split + Copy Summary gate): allowlist the two new user-activity
-- events — copy_summary ({surface:'pipe-tooling'|'email-summary', mode}) fired
-- on every successful clipboard write of either copy surface, and
-- unscaled_ft_block ({surface, flaggedPages} from the pre-copy scale gate;
-- {surface:'page-switch'} when the page-switch re-check disarms a line tool).
-- Re-creates public.log_user_event from the latest deployed body
-- (20260810120000_user_activity_restore_events.sql — the T1-01 migration,
-- itself built on 20260326230000; copying an older body would silently
-- un-allowlist the restore events) with the two events added. Re-asserts the
