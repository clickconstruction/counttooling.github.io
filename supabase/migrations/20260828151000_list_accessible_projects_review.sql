-- Surface the bid-review handoff on the project listing: review_status,
-- review_requested_at, reviewed_at ride along so the Bid Board can lane
-- "Ready for review" bids and badge reviewed ones (and Load Project can badge
-- rows). Return signature changes, so drop first.
